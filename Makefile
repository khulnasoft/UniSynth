...
# We only test core because there are no other tests and unisynth-napi breaks the build
test:
	cargo test --manifest-path packages/unisynth/src/optimizer/core/Cargo.toml

test-update:
	if ! cargo test --manifest-path packages/unisynth/src/optimizer/core/Cargo.toml; then \
		cd packages/unisynth/src/optimizer/core/src/snapshots/; \
