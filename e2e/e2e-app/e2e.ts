// Unisynth E2E orchestration script
...
// Update this array when Unisynth adds new targets

const packages = [
  '@khulnasoft.com/e2e-alpine',
  '@khulnasoft.com/e2e-app-qwik-output',
  '@khulnasoft.com/e2e-app-vue3-output',
  '@khulnasoft.com/e2e-angular',
  '@khulnasoft.com/e2e-qwik',
  '@khulnasoft.com/e2e-react',
  '@khulnasoft.com/e2e-solid',
  '@khulnasoft.com/e2e-svelte',
  '@khulnasoft.com/e2e-vue2',
  '@khulnasoft.com/e2e-vue3',
];
...
async function detectFailures(allResults: any[]) {
  ...
  // allowed so that the case and progress can be merged, even where Unisynth
...
async function main() {
  ...
  for (const c of cases) {
    ...
    // https://github.com/khulnasoft/unisynth/issues/494
    ...
    // Unisynth all targets - with a workaround to tolerate failure, until:
    // https://github.com/khulnasoft/unisynth/issues/510
    await yarn('workspace', '@khulnasoft.com/e2e-app', 'run', 'run-unisynth-separately');
    ...
    try {
      await yarn('workspace', '@khulnasoft.com/e2e-app', 'run', 'playwright');
    } catch (e) {
