...
import { createDocument } from '../../../packages/unisynth/src/testing/document';

export function scaffoldUnisynthProject(): { tmpDir: string; cleanupFn: () => void } {
  ...
  try {
    const tmpDir = runCreateUnisynthCommand(tmpHostDirData.path);
...
function getTmpDirSync(tmpDirOverride?: string) {
  ...
  if (tmpDirOverride) {
    const p = join(tmpDirOverride, 'unisynth_e2e');
    if (existsSync(p)) {
  ...
  return { path: dirSync({ prefix: 'unisynth_e2e' }).name, overridden: false };
}

function runCreateUnisynthCommand(tmpDir: string): string {
  const appDir = 'e2e-app';
  execSync(
    `node "${workspaceRoot}/packages/create-unisynth/create-unisynth.cjs" playground "${join(tmpDir, appDir)}"`
  );
...
function _computeWorkspaceRoot(cwd: string) {
  ...
  if (existsSync(packageJsonAtCwd)) {
    ...
    if (content.name === 'unisynth-monorepo') {
      return cwd;
    }
  }
