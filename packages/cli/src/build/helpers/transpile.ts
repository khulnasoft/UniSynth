...
export const transformImports =
  ({ target, options }: { target: Target; options: UnisynthConfig }) =>
  (code: string) => {
    // we start by replacing all `context.lite` imports with `context`
    // This Context replace is only needed for non-unisynth components, i.e. plain `.js`/`.ts` files.
    // Unisynth components have logic that transform context import paths correctly.
