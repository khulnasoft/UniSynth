...
const help = (toolbox: Toolbox) =>
  toolbox.print.info(
    ...
    `
unisynth command line component processor [version ${toolbox.meta.version()}]

USAGE
	unisynth compile --to=<format> [options] [files]
	unisynth compile -t=<format> [options] [files]
...
OPTIONS
	...
	--config=<file>, -c=<file>
		Specify config file. Defaults to 'unisynth.config.js'.
OUTPUT OPTIONS
	--out=<file>, -o=<file>
	...
	--out-dir=<dir>
		...
		The command "unisynth compile -t react --out-dir lib -- src/*.tsx" would
	...
	--header=<string>
		...
		output is JSON.
	--khulnasoft-components
		Compiled output should will include khulnasoft components where
		available. Useful if you're outputing unisynth that will run
		in Khulnasoft.
...
async function run(argv: any) {
  ...
  const cli = build()
    .brand('unisynth')
    .src(__dirname)
    .plugins('./node_modules', { matching: 'unisynth-*', hidden: true })
