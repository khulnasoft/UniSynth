...
const getTargets = (unisynthConfig: UnisynthConfig, cliOpts: GluegunParameters['options']) => {
  ...
  const targets = Array.from(new Set([...unisynthConfig.targets, ...targetsFromCli])).filter(
...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    const config = getUnisynthConfig(configRelPath);
    if (!config) {
      throw new Error(`No config file found for Unisynth.`);
    }
