...
import { getUnisynthConfig } from '../helpers/get-unisynth-config';
...
const command: GluegunCommand = {
  ...
  run: async (toolbox) => {
    ...
    // Flags and aliases
    const from_ = strings.camelCase(opts.f ?? opts.from ?? 'unisynth');
    ...
    if (!opts.khulnasoftComponents) {
      plugins.push(compileAwayKhulnasoftComponents());
    }

    const unisynthConfig = getUnisynthConfig(configRelPath);
    const generatorOptions = unisynthConfig?.options?.[to as keyof GeneratorOptions];
    ...
    for await (const { data, path } of readFiles()) {
      ...
      try {
        ...
        switch (from_) {
          ...
          case 'khulnasoft':
            json = khulnasoftContentToUnisynthComponent(JSON.parse(data!));
            break;
