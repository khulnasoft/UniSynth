import { UnisynthConfig } from '@khulnasoft.com/unisynth';
import fs from 'fs';
 ...
 */
export function getUnisynthConfig(relPath?: string): UnisynthConfig | null {
  if (!relPath) {
    const path = resolve(process.cwd(), 'unisynth.config');
