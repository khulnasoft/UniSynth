...
import { khulnasoftContentToUnisynthComponent } from '../parsers/khulnasoft';
import {
...
import {
    convertKhulnasoftContentToSymbolHierarchy,
    convertKhulnasoftElementToUnisynthComponent,
...
describe('qwik', () => {
  ...
  test('svg', async () => {
    const component = khulnasoftContentToUnisynthComponent(require('./qwik/specs/qwik.test.svg.json'), {
  ...
  test('Accordion', async () => {
    ...
    compileAwayKhulnasoftComponentsFromTree(component, compileAwayComponents);
  ...
  describe('component', () => {
    test('bindings', async () => {
      // https://khulnasoft.com/content/0937630137c94676ba24f95d9d12e426/edit
      // https://cdn.khulnasoft.com/api/v2/content/page/0937630137c94676ba24f95d9d12e426?apiKey=23dfd7cef1104af59f281d58ec525923&noTraverse=false&preserveAllFields=true&single=true&cachebust=true
      const content = require('./qwik/specs/qwik.test.component-binding.json');
      const state: Record<string, any> = {};
      const hierarchy = convertKhulnasoftContentToSymbolHierarchy(content, {
        collectComponentState: state,
      });
      expect(state).toMatchSnapshot();
      const fileSet = createFileSet({ output: 'mjs', jsx: true });
      hierarchy.depthFirstSymbols.forEach((khulnasoftComponent) => {
        const unisynthComponent = convertKhulnasoftElementToUnisynthComponent(khulnasoftComponent);
        unisynthComponent && addComponent(fileSet, unisynthComponent, { isRoot: false });
      });

      const component = khulnasoftContentToUnisynthComponent(content, {
        includeKhulnasoftExtras: true,
        preserveTextBlocks: true,
      });
      compileAwayKhulnasoftComponentsFromTree(component, compileAwayComponents);
  ...
  test('bindings', async () => {
    // https://khulnasoft.com/content/5d5a2d612df542978577d83c0aefad1e
    // https://cdn.khulnasoft.com/api/v2/content/page/5d5a2d612df542978577d83c0aefad1e?apiKey=23dfd7cef1104af59f281d58ec525923
