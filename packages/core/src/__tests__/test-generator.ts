...
import { UnisynthComponent, createTypescriptProject, parseSvelte } from '..';
...
const basicUnisynth = getRawFile('./data/basic-custom-unisynth-package.raw.tsx');
...
const khulnasoftRenderContent = getRawFile('./data/blocks/khulnasoft-render-content.raw.tsx');
...
const BASIC_TESTS: Tests = {
  ...
  'import types': khulnasoftRenderContent,
  subComponent,
  nestedStyles,
...
export const runTestsForJsx = () => {
  test('Remove Internal unisynth package', async () => {
    const component = parseJsx((await basicUnisynth).code, {
      compileAwayPackages: ['@dummy/custom-unisynth'],
    });
...
export const runTestsForTarget = <X extends BaseTranspilerOptions>({
  ...
  type ParserConfig = {
    name: 'jsx' | 'svelte';
    parser: (args: { code: string; filePath: string }) => Promise<UnisynthComponent>;
    testsArray?: Tests[];
  };
