...
import { checkIsForNode, ForNode, UnisynthNode } from '@/types/unisynth-node';
...
export const checkIsComponentNode = (node: UnisynthNode): boolean =>
  node.name === '@khulnasoft.com/unisynth/component';
...
const replaceInputRefs = curry((json: UnisynthComponent, str: string) => {
...
const getStateObjectString = (json: UnisynthComponent) =>
  flow(
...
const mappers: {
  [key: string]: (json: UnisynthNode, options: ToAlpineOptions) => string;
} = {
  For: (json, options) =>
    !(
      checkIsForNode(json) &&
...
// TODO: spread support
const blockToAlpine = (json: UnisynthNode | ForNode, options: ToAlpineOptions = {}): string => {
