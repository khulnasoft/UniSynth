...
import { Binding, UnisynthNode, checkIsForNode } from '@/types/unisynth-node';
...
const preprocessCssAsJson = (json: UnisynthComponent) => {
...
const stringifyBinding =
  (node: UnisynthNode, options: ToAngularOptions, blockOptions: AngularBlockOptions) =>
...
const handleNgOutletBindings = (node: UnisynthNode, options: ToAngularOptions) => {
  let allProps = '';
...
export const blockToAngular = ({
  ...
  if (checkIsForNode(json)) {
    ...
    str += `</ng-container>`;
    // else condition
    if (isUnisynthNode(json.meta?.else)) {
      str += `<ng-container *ngIf="!(${condition})">`;
...
const traverseToGetAllDynamicComponents = (
  ...
  let dynamicTemplate = '';
  traverse(json).forEach((item) => {
    if (isUnisynthNode(item) && item.name.includes('.') && item.name.split('.').length === 2) {
...
const handleProperties = (json: UnisynthComponent, item: UnisynthNode, index: number) => {
...
export const componentToAngular: TranspilerGenerator<ToAngularOptions> =
  ...
  ({ component: _component }) => {
    ...
    ${renderPreComponent({
      ...
      component: json,
      target: 'angular',
      excludeUnisynthComponents: !options.standalone && !options.preserveImports,
