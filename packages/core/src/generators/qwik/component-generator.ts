...
import { File, SrcKhulnasoft, arrowFnBlock, invoke } from './src-generator';
...
function emitExports(file: File, component: UnisynthComponent) {
...
function emitUseClientEffect(file: File, component: UnisynthComponent) {
...
function emitUseMount(file: File, component: UnisynthComponent) {
...
function emitUseTask(file: File, component: UnisynthComponent) {
...
function emitTrackExpressions(src: SrcKhulnasoft, deps?: string) {
  if (!deps) {
    return;
  }
...
function emitJSX(file: File, component: UnisynthComponent, mutable: string[]) {
...
function emitUseContextProvider(file: File, component: UnisynthComponent) {
...
function emitUseContext(file: File, component: UnisynthComponent) {
...
function emitUseOn(file: File, component: UnisynthComponent) {
  component.hooks.onEvent.forEach((hook) => {
    ...
    if (hook.isRoot) {
      const wrappedHandlerFn = `${file.import(file.qwikModule, '$').localName}((${
        ...
        ${hook.code}
      }) as Parameters<typeof useOn>[1]`; // this type hack is needed until https://github.com/khulnasoft/qwik/issues/5398 is fixed
      file.src.emit(
...
function emitUseRef(file: File, component: UnisynthComponent) {
...
function emitUseStyles(file: File, component: UnisynthComponent): string {
...
function emitTypes(file: File, component: UnisynthComponent) {
  if (file.options.isTypeScript) {
    component.types?.forEach((t) => file.src.emit(t, '\n'));
  }
}

function emitImports(file: File, component: UnisynthComponent) {
...
function emitUseComputed(file: File, component: UnisynthComponent) {
