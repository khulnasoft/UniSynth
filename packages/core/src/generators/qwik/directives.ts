...
import { SrcKhulnasoft, iteratorProperty } from './src-generator';

export const DIRECTIVES: Record<
  string,
  string | ((node: UnisynthNode, blockFn: () => void) => void)
> = {
  Show: (node: UnisynthNode, blockFn: () => void) =>
  ...
  For: (_node: UnisynthNode, blockFn: () => void) =>
    function (this: SrcKhulnasoft) {
      ...
      this.jsxExpression(() => {
        ...
        this.isKhulnasoft && this.emit('((');

        this.emit('(', forArgs, ') => {');

        if (this.isKhulnasoft) {
          this.emit(
            'const l={...this,',
        ...
        blockFn();
        this.emit(');}');
        this.isKhulnasoft && this.emit(').bind(l))');
        this.emit(')');
      });
    },
  Image: minify`${Image}`,
...
export function Image(props: ImageProps) {
  ...
  if (image) {
    const isKhulnasoftIoImage = !!(image || '').match(/\.khulnasoft\.io/) && !props.noWebp;
    const isPixel = props.khulnasoftBlock?.id.startsWith('khulnasoft-pixel-');
    const imgProps = {
      src: props.image,
      style:
