...
function emitStateMethods(
  file: File,
  componentState: UnisynthComponent['state'],
  lexicalArgs: string[],
): StateInit {
...
export function getLexicalScopeVars(component: UnisynthComponent) {
  const newLocal = [
    'props',
    'state',
...
export function getStateMethodsAndGetters(state: UnisynthComponent['state']): MethodMap {
