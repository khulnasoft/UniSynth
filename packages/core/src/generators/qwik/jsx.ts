...
import { File, SrcKhulnasoft, invoke, lastProperty, quote } from './src-generator';

/**
 * Convert a Unisynth nodes to a JSX nodes.
 *
...
export function renderJSXNodes(
  ...
  return function (this: SrcKhulnasoft) {
    ...
    children.forEach((child) => {
      ...
      if (isTextNode(child)) {
        ...
        if (typeof directive == 'function') {
          const blockFn = unisynthNodeToRenderBlock(child.children);
          const meta = child.meta;
          Object.keys(meta).forEach((key) => {
            const value = meta[key];
            if (isUnisynthNode(value)) {
              (blockFn as any)[key] = unisynthNodeToRenderBlock([value]);
            }
          });
    ...
    function unisynthNodeToRenderBlock(children: UnisynthNode[]) {
      return () => {
        ...
        childNeedsFragment && srcKhulnasoft.jsxBeginFragment(fragmentSymbol);
        renderJSXNodes(file, directives, handlers, children, styles, null, {}, false).call(
          srcKhulnasoft,
        );
        childNeedsFragment && srcKhulnasoft.jsxEndFragment();
      };
    }
  };
}
...
function isEmptyTextNode(child: UnisynthNode) {
  return child.properties._text?.trim() == '';
}

function isTextNode(child: UnisynthNode) {
...
function isSlotProjection(child: UnisynthNode) {
    ...
    if (Object.prototype.hasOwnProperty.call(bindings, key)) {
      ...
      if (key == 'css') {
        ...
        bindingExpr = invoke(file.import(file.qwikModule, 'qrl'), [
          ...
          quote(handlerBlock),
          file.options.isKhulnasoft ? '[s,l]' : '[state]',
        ]) as any;
...
function isInlinedDirective(node: UnisynthNode) {
  return (isUnisynthNode(node) && node.name == 'Show') || node.name == 'For';
}
