...
/**
 * If the root Unisynth component only has 1 child, and it is a `Show`/`For` node, then we need to wrap it in a fragment.
 * Otherwise, we end up with invalid React render code.
 */
const isRootSpecialNode = (json: UnisynthComponent) =>
  json.children.length === 1 && ['Show', 'For'].includes(json.children[0].name);

const getRefsString = (json: UnisynthComponent, refs: string[], options: ToReactOptions) => {
...
function provideContext(json: UnisynthComponent, options: ToReactOptions): string | void {
...
function getContextString(component: UnisynthComponent, options: ToReactOptions) {
  let str = '';
...
const getDefaultImport = (json: UnisynthComponent, options: ToReactOptions): string => {
...
const getPropsDefinition = ({ json }: { json: UnisynthComponent }) => {
...
const isRSC = (json: UnisynthComponent, options: ToReactOptions) => {
...
const checkShouldAddUseClientDirective = (json: UnisynthComponent, options: ToReactOptions) => {
...
const _componentToReact = (
  ...
  // Always use state if we are generate Khulnasoft react code
  const hasState = options.stateType === 'khulnasoft' || checkHasState(json);
    ...
    ${
      hasState
        ? options.stateType === 'mobx'
          ...
          : options.stateType === 'khulnasoft'
          ? `const state = useKhulnasoftState(${getStateObjectStringFromComponent(json)});`
