...
import { BaseNode, Binding, ForNode, UnisynthNode } from '@/types/unisynth-node';
...
${json.children.map((item) => blockToSvelte({ json: item, options, parentComponent })).join('\n')}
  ...
  {:else}
  ${blockToSvelte({
    json: json.meta.else as UnisynthNode,
    options,
    parentComponent,
  })}
  `
...
const BINDINGS_MAPPER = {
  innerHTML: (json: UnisynthNode, options: ToSvelteOptions) =>
...
const getTagName = ({
  ...
  json: UnisynthNode;
  parentComponent: UnisynthComponent;
...
type BlockToSvelte<T extends BaseNode = UnisynthNode> = (props: {
  json: T;
  options: ToSvelteOptions;
  parentComponent: UnisynthComponent;
}) => string;
...
const stringifyBinding =
  (node: UnisynthNode, options: ToSvelteOptions) =>
