...
// TODO: px to 2 px
export const collectTaroStyles = (json: UnisynthComponent): ClassStyleMap => {
  const styleMap: ClassStyleMap = {};

  const componentIndexes: { [className: string]: number | undefined } = {};

  traverse(json).forEach(function (item) {
    if (!isUnisynthNode(item) || typeof item.bindings.css?.code !== 'string') {
      return;
    }
...
const PROCESS_TARO_PLUGIN: Plugin = () => ({
  json: {
    pre: (json: UnisynthComponent) => {
      const TaroComponentsImports: UnisynthImport = { path: '@tarojs/components', imports: {} };
