...
import { ContextGetInfo, ContextSetInfo, UnisynthComponent } from '@/types/unisynth-component';
...
export const addPropertiesToJson =
  (properties: UnisynthNode['properties']) =>
  (json: UnisynthNode): UnisynthNode => ({
    ...json,
    properties: {
      ...json.properties,
      ...properties,
...
export const addBindingsToJson =
  (bindings: UnisynthNode['bindings']) =>
  (json: UnisynthNode): UnisynthNode => ({
    ...json,
    bindings: {
      ...json.bindings,
...
export const mapUnisynthComponentToKebabCase = (componentName: string) =>
  componentName.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();

// Transform <FooBar> to <foo-bar> as Vue2 needs
export const renameUnisynthComponentsToKebabCase = (str: string) =>
  str.replace(/<\/?\w+/g, (match) => {
    const tagName = match.replaceAll('<', '').replaceAll('/', '');
    if (VALID_HTML_TAGS.includes(tagName)) {
      return match;
    } else {
      return mapUnisynthComponentToKebabCase(match);
    }
  });

export function getContextNames(json: UnisynthComponent) {
...
const getAllRefs = (component: UnisynthComponent) => {
...
function prefixMethodsWithThis(input: string, component: UnisynthComponent, options: ToVueOptions) {
