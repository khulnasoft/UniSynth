...
import { getOnUpdateHookName, processBinding, renameUnisynthComponentsToKebabCase } from './helpers';
import { generateOptionsApiScript } from './optionsApi';
import { ToVueOptions } from './types';

// Transform <foo.bar key="value" /> to <component :is="foo.bar" key="value" />
function processDynamicComponents(json: UnisynthComponent, _options: ToVueOptions) {
...
function processForKeys(json: UnisynthComponent, _options: ToVueOptions) {
  traverse(json).forEach((node) => {
    if (isUnisynthNode(node)) {
      if (node.name === 'For') {
  ...
  ({ component: _component, path }) => {
    ...
    const template =
      options.casing === 'kebab'
        ? renameUnisynthComponentsToKebabCase(templateStrBody)
        : templateStrBody;
