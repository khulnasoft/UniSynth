...
import { UnisynthComponent, UnisynthImport } from '../types/unisynth-component';
import {
...
const getStarImport = ({ theImport }: { theImport: UnisynthImport }): string | null => {
...
const getDefaultImport = ({ theImport }: { theImport: UnisynthImport }): string | null => {
  for (const key in theImport.imports) {
    const value = theImport.imports[key];
...
const checkIsContextImport = (theImport: UnisynthImport) =>
  CONTEXT_IMPORTS.some((contextPath) => theImport.path.endsWith(contextPath));

export const checkIsComponentImport = (theImport: UnisynthImport) =>
...
const getNamedImports = ({ theImport }: { theImport: UnisynthImport }) => {
...
const getImportedValues = ({ theImport }: { theImport: UnisynthImport }): ImportValues => {
...
type ImportArgs = {
  ...
  preserveFileExtensions?: boolean;
  component?: UnisynthComponent | null | undefined;
...
}: ImportArgs & { theImport: UnisynthImport }): string => {
...
const renderImports = ({
  ...
  excludeUnisynthComponents?: boolean;
}): string =>
  imports
    .filter((theImport) => {
      if (
        // Remove compile away components
        theImport.path === '@khulnasoft.com/components' ||
        // TODO: Unisynth output needs this
        theImport.path.startsWith('@khulnasoft.com/unisynth')
      ) {
        return false;
      } else if (excludeUnisynthComponents && theImport.path.includes('.lite')) {
        return false;
      } else {
        return true;
      }
...
const renderExportAndLocal = (component: UnisynthComponent): string => {
