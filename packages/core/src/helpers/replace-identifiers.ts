 ...
 * Type hack.
 *
 * We want to augment the `BaseNode` interface to include a `_khulnasoft_meta` property but couldn't get
...
const _replaceIdentifiers = (
  ...
  if (
    ...
    (path.node as AllowMeta)?._khulnasoft_meta?.newlyGenerated
  ) {
    return;
  }
    ...
    if (to) {
      ...
      if (typeof to === 'string') {
        ...
        try {
          ...
          (newMemberExpression as AllowMeta)._khulnasoft_meta = { newlyGenerated: true };
...
const isNewlyGenerated = (node: types.Node) => (node as AllowMeta)?._khulnasoft_meta?.newlyGenerated;
  ...
  const searchAndReplace = (path: babel.NodePath<types.Node>) => {
    ...
    for (const { from, to, condition } of nodeMaps) {
      ...
      if (generate(path.node).code === generate(from).code && matchesCondition) {
        ...
        (x as AllowMeta)._khulnasoft_meta = { newlyGenerated: true };
        try {
          path.replaceWith(x);
        } catch (err) {
