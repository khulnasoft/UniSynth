...
import { mapImportDeclarationToUnisynthImport } from '../unisynth-imports';
...
export const getSignalUnisynthImportForTarget = (target: Target) => {
  const signalType = getSignalMappingForTarget(target);
  if (!signalType) {
    return undefined;
  }
  return mapImportDeclarationToUnisynthImport(signalType.importDeclaration);
};
    ...
    ImportSpecifier(path) {
      if (types.isIdentifier(path.node.imported) && path.node.imported.name === 'Signal') {
        if (
          ...
          path.parentPath.node.source.value === '@khulnasoft.com/unisynth'
        ) {
          /**
           * in case the import is aliased, we need to use the local name,
           * e.g. `import { Signal as MySignal } from '@khulnasoft.com/unisynth'`
           */
