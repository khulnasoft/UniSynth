...
import { UnisynthExports } from '../../types/unisynth-component';
import { isTypeOrInterface } from './component-types';
import { isImportOrDefaultExport } from './helpers';

const { types } = babel;

export const generateExports = (path: babel.NodePath<babel.types.Program>): UnisynthExports => {
  ...
  return exportsOrLocalVariables.reduce<UnisynthExports>((pre, node) => {
    let name, isFunction;
