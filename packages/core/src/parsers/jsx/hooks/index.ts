...
import { ParseUnisynthOptions } from '../types';
...
export function parseDefaultPropsHook(
  ...
  if (types.isObjectExpression(firstArg)) {
    component.defaultProps = parseStateObjectToUnisynthState(firstArg, false);
  }
}
...
export const collectModuleScopeHooks =
  (component: UnisynthComponent, options: ParseUnisynthOptions) => (nodes: babel.types.Statement[]) =>
    nodes.filter((node) => {
