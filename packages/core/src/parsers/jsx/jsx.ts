...
/**
 * This function takes the raw string from a Unisynth component, and converts it into a JSON that can be processed by
 * each generator function.
 *
 * @param jsx string representation of the Unisynth component
 * @returns A JSON representation of the Unisynth component
 */
export function parseJsx(
  jsx: string,
  _options: Partial<ParseUnisynthOptions> = {},
): UnisynthComponent {
  let subComponentFunctions: string[] = [];

  const options: ParseUnisynthOptions = {
    typescript: false,
    ..._options,
  };
      ...
      (): babel.PluginObj<Context> => ({
        visitor: {
          ...
          Program(path, context) {
            ...
            context.khulnasoft = {
              component: createUnisynthComponent(),
            };
            ...
            context.khulnasoft.component.exports = generateExports(path);
            ...
            const preComponentCode = pipe(
              ...
              collectModuleScopeHooks(context.khulnasoft.component, options),
              types.program,
              generate,
            ...
            context.khulnasoft.component.hooks.preComponent = { code: preComponentCode };
              ...
              if (name[0].toUpperCase() === name[0]) {
                path.traverse({
                  ...
                  CallExpression(path) {
                    ...
                    const blockId = getTargetId(context.khulnasoft.component);
                    ...
                    context.khulnasoft.component.targetBlocks = {
                      ...context.khulnasoft.component.targetBlocks,
                      [blockId]: targetBlock,
                    };
                  },
  ...
  const stringifiedUnisynthComponent = stripNewlinesInStrings(
    output.code
      .trim()
  ...
  const unisynthComponent: UnisynthComponent = tryParseJson(stringifiedUnisynthComponent);

  mapStateIdentifiers(unisynthComponent, stateToScope);
  extractContextComponents(unisynthComponent);

  unisynthComponent.subComponents = subComponentFunctions.map((item) => parseJsx(item, options));

  const signalTypeImportName = getSignalImportName(jsxToUse);

  if (signalTypeImportName) {
    unisynthComponent.signals = { signalTypeImportName };
  }
  ...
  if (options.tsProject && options.filePath) {
    ...
    optionalProps.forEach((prop) => {
      unisynthComponent.props = {
        ...unisynthComponent.props,
        [prop]: {
          ...unisynthComponent.props?.[prop]!,
          optional: true,
        },
      };
    });
    ...
    reactiveValues.state.forEach((state) => {
      if (!unisynthComponent.state[state]) return;
      unisynthComponent.state[state]!.propertyType = 'reactive';
    });

    reactiveValues.context.forEach((context) => {
      if (!unisynthComponent.context.get[context]) return;
      unisynthComponent.context.get[context].type = 'reactive';
    });
  }

  traverseNodes(unisynthComponent, (node) => {
    node.children = node.children.filter(filterEmptyTextNodes);
  });

  return unisynthComponent;
}
