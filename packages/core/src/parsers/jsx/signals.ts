...
const UNISYNTH_IMPORT_PATHS = [
  // actual production path
  '/node_modules/@khulnasoft.com/unisynth/',
  // possible path if symlinking unisynth locally
  '/unisynth/packages/core/',
];
...
export const findSignals = ({ filePath, project }: { project: Project; filePath: string }) => {
  ...
  const checkIsSignalSymbol = (type: Type<ts.Type>) => {
    ...
    if (UNISYNTH_IMPORT_PATHS.some((path) => parent.getName().includes(path))) {
      return true;
    }
