...
import { UnisynthNode } from '@khulnasoft.com/unisynth';
...
const consolidateClassBindings = (item: UnisynthNode) => {
...
/**
 * Convert state identifiers from React hooks format to the state.* format Unisynth needs
 * e.g.
 *   text -> state.text
 *   setText(...) -> state.text = ...
 *
 ...
 * })`
 */
export function mapStateIdentifiers(json: UnisynthComponent, stateProperties: string[]) {
  ...
  traverse(json).forEach(function (item) {
    ...
    if (isUnisynthNode(item) && item.name.toLowerCase() === item.name) {
...
export const parseStateObjectToUnisynthState = (
  object: ObjectExpression,
  isState: boolean = true, // parse state or defaultProps
): UnisynthState => {
  const state: UnisynthState = {};
  object.properties.forEach((x) => {
    if (isSpreadElement(x)) {
      throw new Error('Parse Error: Unisynth cannot consume spread element in state object: ' + x);
    }

    if (isPrivateName(x.key)) {
      throw new Error('Parse Error: Unisynth cannot consume private name in state object: ' + x.key);
    }

    if (!isIdentifier(x.key) && !isStringLiteral(x.key)) {
      throw new Error(
        'Parse Error: Unisynth cannot consume non-identifier and non-string key in state object: ' +
          x.key,
      );
    }
