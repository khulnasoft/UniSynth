...
import type { BaseHook, StateValue } from '../../../types/unisynth-component';
...
function addPropertiesAndStateToNode(json: SveltosisComponent, node: UnisynthNode) {
...
function postProcessChildren(json: SveltosisComponent, children: UnisynthNode[]) {
  for (const node of children) {
    addPropertiesAndStateToNode(json, node);
    processBindings(json, node);

    let children: UnisynthNode[] = [];

    if (node.children?.length > 0) {
      children = node.children;
    }

    const metaValues = (Object.values(node.meta) || []) as Array<UnisynthNode | UnisynthNode['meta']>;

    if (metaValues.length > 0) {
      const metaChildren = metaValues.filter((item) => {
        return item?.['@type'] === '@khulnasoft.com/unisynth/node';
      }) as UnisynthNode[];
