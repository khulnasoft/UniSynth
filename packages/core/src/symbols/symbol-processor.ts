...
import {
    createKhulnasoftElement,
    isKhulnasoftElement,
    khulnasoftContentToUnisynthComponent,
} from '@/parsers/khulnasoft';
...
export type SymbolHierarchy = {
  ...
  depthFirstSymbols: KhulnasoftElement[];
} & {
  [id: string]: string[];
};
...
/**
 * Ensure every symbol in a KhulnasoftContent tree has a unique ID.
 * Mutates the data tree directly.
 */
export function ensureAllSymbolsHaveIds(content: KhulnasoftContent): void {
  let counter = 0;
...
export function convertKhulnasoftContentToSymbolHierarchy(
    ...
    if (isKhulnasoftElement(el)) {
      if (el?.component?.name === 'Symbol') {
        if (collectComponentState) {
          const symbol: KhulnasoftSymbol = el.component.options.symbol;
...
export function convertKhulnasoftElementToUnisynthComponent(
  element: KhulnasoftElement,
): UnisynthComponent | null {
  ...
  element.children = [
    createKhulnasoftElement({
      ...
      properties: {
        'khulnasoft-content-id': id,
      },
    }),
  ];

  const unisynthComponent: UnisynthComponent = {
    ...khulnasoftContentToUnisynthComponent(elContent, {
      includeKhulnasoftExtras: true,
      preserveTextBlocks: true,
    }),
    name: componentName,
  };
  unisynthComponent.meta.khulnasoftElId = element.id;
...
function getIdFromSymbol(el: KhulnasoftElement): string {
