...
import type { CreateAppResult } from 'packages/unisynth/src/cli/types';
...
import { installDeps as installDepsFn } from 'packages/unisynth/src/cli/utils/install-deps';
...
function parseArgs(args: string[], templates: string[]) {
  const parsedArgs = yargs(args)
    .strict()
    .command('* <template> <outDir>', 'Create a new project powered by unisynth', (yargs) => {
      return yargs
        .positional('template', {
          type: 'string',
        ...
        .usage('npm create unisynth@latest base ./my-project <options>');
...
export async function runCreateCli(...args: string[]): Promise<CreateAppResult> {
  ...
  intro(`Let's create a ${bgBlue(' Unisynth App ')} ✨ (v${(globalThis as any).UNISYNTH_VERSION})`);
