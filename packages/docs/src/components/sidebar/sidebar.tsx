...
export const SideBar = component$((props: { allOpen?: boolean }) => {
  ...
  const allOpen = url.pathname.startsWith('/unisynthcity/') || props.allOpen;

  useOnDocument(
    'DOMContentLoaded',
    sync$(() => {
      try {
        const val = sessionStorage.getItem('unisynth-sidebar');
        const savedScroll = !val || /null|NaN/.test(val) ? 0 : +val;
        const el = document.getElementById('unisynth-sidebar');
        if (el) {
  ...
  return (
    <aside class="sidebar">
      <nav id="unisynth-sidebar" class="menu">
        <button
        ...
        <Items
          ...
          onClick$={sync$(() => {
            try {
              const scrollTop = document.getElementById('unisynth-sidebar')!.scrollTop;
              sessionStorage.setItem('unisynth-sidebar', String(scrollTop));
            } catch (err) {
              //
            }
          })}
