...
// export const unisynthGPT = server$(async function* (query: string) {
...
//           role: 'system',
//           content:
//             'You are UnisynthGPT, your job is to answer questions about Unisynth, a new javascript framework focused on instant interactivity and server-side rendering.\nRelevant Unisynth documentation and the user question will be provided. Try to answer the question in a short and concise way.',
//         },
//         {
//           role: 'user',
//           content: docsStr,
...
export async function resolveContext(docsData: any[]) {
  ...
  try {
    for (const result of docsData) {
      ...
      const url = `https://raw.githubusercontent.com/khulnasoft/unisynth/${commit_hash}/${file_path}`;
      if (!files.has(url)) {
        files.set(
          url,
    ...
    for (const [url, ranges] of Object.entries(docsRanges)) {
      ...
      if (lines.length > 0) {
        const parts = new URL(url).pathname
          ...
          .join('/');
        const docsURL = `https://unisynth.dev/${parts}/`;
