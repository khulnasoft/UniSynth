...
import type { Diagnostic } from '@khulnasoft.com/unisynth/optimizer';
...
// import { UNISYNTH_REPL_DEPS_CACHE } from './worker/repl-constants';
const UNISYNTH_REPL_DEPS_CACHE = 'UnisynthReplDeps';
...
export const addUnisynthLibs = async (version: string) => {
  const monaco = await getMonaco();
  ...
  typescriptDefaults.addExtraLib(
    `declare module '@khulnasoft.com/unisynth/jsx-runtime' { export * from '@khulnasoft.com/unisynth' }`,
    '/node_modules/@khulnasoft.com/unisynth/dist/jsx-runtime.d.ts'
  );
  typescriptDefaults.addExtraLib(CLIENT_LIB);
};

const loadDeps = async (unisynthVersion: string) => {
  const [M, m, p] = unisynthVersion.split('-')[0].split('.').map(Number);
  const prefix =
    unisynthVersion === 'bundled' || M > 1 || (M == 1 && (m > 7 || (m == 7 && p >= 2)))
      ? '/dist/'
      : '/';
  const deps: NodeModuleDep[] = [
    // unisynth
    {
      pkgName: '@khulnasoft.com/unisynth',
      pkgVersion: unisynthVersion,
  ...
  const cache = await caches.open(UNISYNTH_REPL_DEPS_CACHE);
