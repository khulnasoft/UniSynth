import { component$, useComputed$ } from '@khulnasoft.com/unisynth';
...
export const ReplOutputPanel = component$(({ input, store }: ReplOutputPanelProps) => {
  ...
  const clientBundlesNoCore = useComputed$(() =>
    // Unisynth Core is not interesting and is large, slowing down the UI
    store.clientBundles.filter((b) => !b.path.endsWith('unisynthCore.js'))
  );
