...
import { UNISYNTH_PKG_NAME, bundled } from './bundled';

const bundledVersion = bundled[UNISYNTH_PKG_NAME].version;
...
export const getReplVersion = async (version: string | undefined, offline: boolean) => {
  ...
  try {
    ...
    if (!offline && isExpiredNpmData(npmData)) {
      ...
      console.debug(`Unisynth REPL, fetch npm data: ${UNISYNTH_NPM_DATA}`);
      const npmRsp = await fetch(UNISYNTH_NPM_DATA);
      npmData = await npmRsp.json();
      npmData!.timestamp = Date.now();

      localStorage.setItem(NPM_STORAGE_KEY, JSON.stringify(npmData));
    } else {
      console.debug(`Unisynth REPL, using cached npm data`);
    }
  } catch (e) {
...
const UNISYNTH_NPM_DATA = `https://data.jsdelivr.com/v1/package/npm/@khulnasoft.com/unisynth`;

const NPM_STORAGE_KEY = `unisynthNpmData`;

// https://data.jsdelivr.com/v1/package/npm/@khulnasoft.com/unisynth
interface NpmData {
