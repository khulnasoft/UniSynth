...
import { UNISYNTH_PKG_NAME, bundled, getNpmCdnUrl } from './bundled';
...
const getDependencies = (input: ReplAppInput) => {
  ...
  if (input.version !== 'bundled') {
    ...
    for (const p of [
      ...
      `${prefix}server.cjs`,
      `/bindings/unisynth.wasm.cjs`,
      `/bindings/unisynth_wasm_bg.wasm`,
    ]) {
      out[UNISYNTH_PKG_NAME][p] = getNpmCdnUrl(bundled, UNISYNTH_PKG_NAME, input.version, p);
    }
  }
  return out;
};
