...
import type { NoSerialize } from '@khulnasoft.com/unisynth';
...
export interface ReplInputOptions extends Omit<UnisynthRollupPluginOptions, 'srcDir'> {
  buildId: number;
...
export interface ReplResult extends ReplMessageBase {
  ...
  clientBundles: ReplModuleOutput[];
  ssrModules: ReplModuleOutput[];
  manifest: UnisynthManifest | undefined;
  diagnostics: Diagnostic[];
  events: ReplEvent[];
}
