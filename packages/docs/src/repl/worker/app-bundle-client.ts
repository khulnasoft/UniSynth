...
import type { Diagnostic, UnisynthRollupPluginOptions } from '@khulnasoft.com/unisynth/optimizer';
...
export const appBundleClient = async (
  ...
  const unisynthRollupClientOpts: UnisynthRollupPluginOptions = {
    target: 'client',
  ...
  console.debug('client opts', unisynthRollupClientOpts);
  ...
  const rollupInputOpts: InputOptions = {
    ...
    plugins: [
      replCss(options),
      self.unisynthOptimizer?.unisynthRollup(unisynthRollupClientOpts),
      ...
      result.clientBundles.map(async (b) => {
        ...
        const rsp = new Response(b.code, {
          headers: {
            ...
            'X-Unisynth-REPL-App': 'ssr-result',
          },
        });
