...
export const appBundleSsr = async (options: ReplInputOptions, result: ReplResult) => {
  ...
  const unisynthRollupSsrOpts: UnisynthRollupPluginOptions = {
    target: 'ssr',
  ...
  console.debug('ssr opts', unisynthRollupSsrOpts);
  ...
  const rollupInputOpts: InputOptions = {
    ...
    plugins: [
      replCss(options),
      self.unisynthOptimizer?.unisynthRollup(unisynthRollupSsrOpts),
