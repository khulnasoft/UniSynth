...
export const initReplServer = (win: Window, doc: Document, nav: Navigator) => {
  ...
  if (!/^[a-z0-9]+$/.test(clientId)) {
    console.error('Unisynth REPL server missing valid client id');
    return;
  }
  ...
  const replReady = () => {
    ...
    console.debug('Unisynth REPL server "%s" ready', clientId);
  ...
  if (win.parent === win) {
    console.error('Unisynth REPL server "%s" is not an iframe window', clientId);
  } else {
    loadTmr = setTimeout(() => {
      console.error('Unisynth REPL server "%s" has not initialized', clientId);
    }, 15000);
      ...
      .then(
        (reg) => {
          ...
          if (swRegistration.active) {
            console.debug('Unisynth REPL server "%s" service worker registration active', clientId);
            replReady();
