---
title: \@khulnasoft.com/unisynth-city/static API Reference
---

# [API](/api) &rsaquo; @khulnasoft.com/unisynth-city/static

## generate

Use this function when SSG should be generated from another module, such as a Vite plugin. This function's should be passed the paths of the entry module and Unisynth City Plan.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth-city/src/static/index.ts)
...
Path to the Unisynth City Plan module exporting the default `@unisynth-city-plan`.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth-city/src/static/types.ts)
...
The URL `origin`, which is a combination of the scheme (protocol) and hostname (domain). For example, `https://unisynth.dev` has the protocol `https://` and domain `unisynth.dev`. However, the `origin` does not include a `pathname`.
...
If the site also starts with a pathname other than `/`, please use the `basePathname` option in the Unisynth City config options.
