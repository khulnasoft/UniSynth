---
title: \@khulnasoft.com/unisynth API Reference
---

# [API](/api) &rsaquo; @khulnasoft.com/unisynth
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/qrl/qrl.public.ts)
...
Unisynth Optimizer marker function.

Use `$(...)` to tell Unisynth Optimizer to extract the expression in `$(...)` into a lazy-loadable resource referenced by `QRL`.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/types/jsx-generated.ts)
...
TS defines these with the React syntax which is not compatible with Unisynth. E.g. `ariaAtomic` instead of `aria-atomic`.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/types/jsx-unisynth-attributes.ts)
...
Type representing the Unisynth component.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/component/component.public.ts)

## component$

Declare a Unisynth component that can be used to create UI.

Use `component$` to declare a Unisynth component. A Unisynth component is a special kind of component that allows the Unisynth framework to lazy load and execute the component independently of other Unisynth components as well as lazy load the component's life-cycle hooks and event handlers.

Side note: You can also declare regular (standard JSX) components that will have standard synchronous behavior.

Unisynth component is a facade that describes how the component should be used without forcing the implementation of the component to be eagerly loaded. A minimum Unisynth definition consists of:
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-task.ts)
...
Use `createContextId()` to create a `ContextId`. A `ContextId` is just a serializable identifier for the context. It is not the context value itself. See `useContextProvider()` and `useContext()` for the values. Unisynth needs a serializable ID for the context so that the it can track context providers and consumers in a way that survives resumability.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-context.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/platform/types.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-signal.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/types/jsx-node.ts)
...
The Unisynth-specific attributes that DOM elements accept

```typescript
export interface DOMAttributes<EL extends Element> extends DOMAttributesBase<EL>, UnisynthEvents<EL>
```

**Extends:** DOMAttributesBase&lt;EL&gt;, UnisynthEvents&lt;EL&gt;
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/error-handling.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/jsx-runtime.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/platform/platform.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/factory.ts)
...
It is very common for functions to take a lazy-loadable resource as a first argument. For this reason, the Unisynth Optimizer automatically extracts the first argument from any function which ends in `$`.

This means that `foo$(arg0)` and `foo($(arg0))` are equivalent with respect to Unisynth Optimizer. The former is just a shorthand for the latter.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/util/implicit_dollar.ts)
...
```typescript
interface IntrinsicAttributes extends UnisynthIntrinsicAttributes
```

**Extends:** UnisynthIntrinsicAttributes
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/state/signal.ts)
...
The names of events that Unisynth knows about. They are all lowercase, but on the JSX side, they are PascalCase for nicer DX. (`onAuxClick$` vs `onauxclick$`)

```typescript
export type KnownEventNames = LiteralUnion<AllEventKeys, string>;
```

[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/types/jsx-unisynth-events.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/state/common.ts)
...
Each Unisynth container needs to include its own prefetch graph.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/components/prefetch.ts)
...
There can only be one service worker per page. Because there can be many separate Unisynth Containers on the page each container needs to load its prefetch graph using `PrefetchGraph` component.
...
- `base` - Base URL for the service worker `import.meta.env.BASE_URL` or `/`. Default is `import.meta.env.BASE_URL` - `scope` - Base URL for when the service-worker will activate. Default is `/` - `path` - Path to the service worker. Default is `unisynth-prefetch-service-worker.js` unless you pass a path that starts with a `/` then the base is ignored. Default is `unisynth-prefetch-service-worker.js` - `verbose` - Verbose logging for the service worker installation. Default is `false` - `nonce` - Optional nonce value for security purposes, defaults to `undefined`.
...
export type PropsOf<COMP> = COMP extends string
  ? COMP extends keyof UnisynthIntrinsicElements
    ? UnisynthIntrinsicElements[COMP]
    : UnisynthIntrinsicElements["span"]
...
**References:** [UnisynthIntrinsicElements](#unisynthintrinsicelements), [FunctionComponent](#functioncomponent), [Component](#component)
...
`QRL` is an opaque token that is generated by the Unisynth Optimizer. (Do not rely on any properties in `QRL` as it may change between versions.)

\#\# Creating `QRL` references

Creating `QRL` is done using `$(...)` function. `$(...)` is a special marker for the Unisynth Optimizer that marks that the code should be extracted into a lazy-loaded symbol.
...
In the above code, the Unisynth Optimizer detects `$(...)` and transforms the code as shown below:
...
NOTE: `qrl(...)` is a result of Unisynth Optimizer transformation. You should never have to invoke this function directly in your application. The `qrl(...)` function should be invoked only after the Unisynth Optimizer transformation.
...
1. `QRL`s must be serializable into HTML. 2. `QRL`s must be resolved by framework relative to `q:base`. 3. `QRL`s must be able to capture lexically scoped variables. 4. `QRL`s encapsulate the difference between running with and without Unisynth Optimizer. 5. `QRL`s allow expressing lazy-loaded boundaries without thinking about chunk and symbol names.
...
These are the main reasons why Unisynth introduces its own concept of `QRL`.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/qrl/qrl.ts)
...
An event handler for Unisynth events, can be a handler QRL or an array of handler QRLs.
...
The Unisynth DOM attributes without plain handlers, for use as function parameters

```typescript
export interface UnisynthAttributes<EL extends Element> extends DOMAttributesBase<EL>, UnisynthEvents<EL, false>
```

**Extends:** DOMAttributesBase&lt;EL&gt;, UnisynthEvents&lt;EL, false&gt;
...
> Warning: This API is now obsolete.
>
> Use `Event` and use the second argument to the handler function for the current event target. Also note that in Unisynth, onInput$ with the InputEvent is the event that behaves like onChange in React.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/types/jsx-unisynth.ts)
...
```typescript
export type UnisynthHTMLElements = {
  [tag in keyof HTMLElementTagNameMap]: Augmented<
    HTMLElementTagNameMap[tag],
    SpecialAttrs[tag]
  > &
    HTMLElementAttrs &
    UnisynthAttributes<HTMLElementTagNameMap[tag]>;
};
```

**References:** [HTMLElementAttrs](#htmlelementattrs), [UnisynthAttributes](#unisynthattributes)

[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/types/jsx-generated.ts)

## UnisynthIdleEvent

Emitted by unisynth-loader on document when the document first becomes idle
...
Emitted by unisynth-loader on document when the document first becomes interactive
...
## UnisynthIntrinsicElements

The interface holds available attributes of both native DOM elements and custom Unisynth elements. An example showing how to define a customizable wrapper component:
...
**Extends:** [UnisynthHTMLElements](#unisynthhtmlelements), [UnisynthSVGElements](#unisynthsvgelements)

[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/types/jsx-unisynth-elements.ts)

## UnisynthInvalidEvent
...
```typescript
export declare namespace UnisynthJSX
```
...
[ElementChildrenAttribute](#unisynthjsx-elementchildrenattribute)
...
[IntrinsicAttributes](#unisynthjsx-intrinsicattributes)
...
[ElementType](#unisynthjsx-elementtype)
...
```typescript
export type UnisynthSVGElements = {
  [K in keyof Omit<
...
Emitted by unisynth-loader when a module was lazily loaded
...
Emitted by unisynth-loader when an element becomes visible. Used by `useVisibleTask$`
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/dom/render.public.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/ssr/render-ssr.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-resource.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/utils.public.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/render/jsx/slot.public.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/container/container.ts)
...
```typescript
export interface SVGProps<T extends Element> extends SVGAttributes, UnisynthAttributes<T>
```

**Extends:** [SVGAttributes](#svgattributes), [UnisynthAttributes](#unisynthattributes)&lt;T&gt;
...
The `Tracker` is passed into the `taskFn` of `useTask`. It is intended to be used to wrap state objects in a read proxy which signals to Unisynth which properties should be watched for changes. A change to any of the properties causes the `taskFn` to rerun.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-core.ts)
...
- When calculating the first time, it will see it's a promise and it will restart the render function. - Unisynth can't track used signals after the first await, which leads to subtle bugs. - Both `useTask$` and `useResource$` are available, without these problems.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-error-boundary.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-id.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-on.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-env-data.ts)
...
Creates an object that Unisynth can track across serializations.
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-store.public.ts)
...
Component styles allow Unisynth to lazy load the style information for the component only when needed. (And avoid double loading it in case of SSR hydration.)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/use/use-styles.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/util/types.ts)
...
[Edit this section](https://github.com/khulnasoft/unisynth/tree/main/packages/unisynth/src/core/version.ts)
...
> Warning: This API is now obsolete.
>
> This is the type for a React Native WebView. It doesn't belong in Unisynth (yet?) but we're keeping it for backwards compatibility.
