...
Every Unisynth application is contained inside a element, usually the `<html>` element. This element becomes the container for the application. The container is the root element for the application and all components, state and events are contained within.
...
Since containers are implicitly rendered by the Unisynth runtime, it's not possible to define custom HTML attributes using JSX, however, the SSR render APIs, like `renderToString` and `renderToStream`, provide the `containerAttributes` option to define custom attributes:
...
Along with the custom attributes, Unisynth will automatically render the `q:container`, `q:version`, `q:render` and `q:base` attributes.

- `q:container` - The container state. This attribute is used by the Unisynth runtime to determine if the container is in a paused state or not. The value of this attribute is either `paused` or `resumed`.

- `q:version` - The version of the Unisynth runtime.
...
- **versioned**: Each container can run a different version of the Unisynth framework. Allowing for the composability of the website from many small containers.
...
<html q:container="paused" q:version="1.9.0" q:base="/build/">
  <head>
    <title>My Unisynth Application</title>
  </head>
  <body>
...
- Components must be compiled together and, as a result, share the same bundle artifacts and same Unisynth version.
...
Containers allow multiple independent Unisynth applications to run on the page and behave as a single application to the user. There are two most common use cases:
...
3. The Unisynth approach treats the navigation and the outlet as two different containers. The first navigation downloads HTML representing the full page (with both containers). The subsequent navigation fetches the HTML only for the outlet container. This approach is the best of both worlds. The navigation is fast (no JavaScript download or execution), and the application keeps its state in the parent container.
