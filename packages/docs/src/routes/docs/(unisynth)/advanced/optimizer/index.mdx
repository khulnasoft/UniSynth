...
Unisynth's philosophy is to delay loading code for as long as possible. To do that, Unisynth relies on an Optimizer to re-arrange the code for lazy loading. The Optimizer is a code level transformation that runs as part of the rollup.
