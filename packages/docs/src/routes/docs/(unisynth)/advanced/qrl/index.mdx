...
QRL (Unisynth URL) is a particular form of URL that Unisynth uses to lazy load content.

QRLs:

- are specially formatted URLs that are left as attributes in the HTML to tell Unisynth where the handlers for the code should be loaded from.
...
Unisynth uses `q:base` to resolve a QRL into an absolute URL if the URL is relative. (If no `q:base` attribute is present, then `document.baseURI` is used as a base.)
...
<html>
  <body q:base="/build/">
    ...
    <script>
      /*Unisynthloader script*/
    </script>
    <script type="unisynth/json">
      {...json...}
    </script>
  </body>
</html>
...
The main thing to note is the `on:click` attribute. This attribute gets read by the Unisynthloader when the user clicks on the button.

1. HTML is loaded in the browser, and Unisynthloader registers a global `click` listener. No other JavaScript is loaded/executed at this point.
2. User clicks on the `<button>`. This fires a `click` event which bubbles up and is handled by the Unisynthloader.
3. Unisynthloader retraces the event bubble path and looks for `on:click` attribute, which it finds on `<button>`.
4. Unisynthloader now tries to load the corresponding chunk. To do that, the Unisynthloader needs to resolve the relative path of `./chunk-c.js`. It uses these values to build an absolute path starting at `<button>` and walking towards the document.
   - `on:click="./chunk-c.js#Counter_onClick[0,1]"`
   - `<body q:base="/build/">`
   - `document.baseURI = "http://localhost/index.html"`
   - The resulting absolute URL is `http://localhost/build/chunk-c.js` which Unisynthloader fetches.
5. Unisynthloader now retrieves the `Counter_onClick` reference from `http://localhost/build/chunk-c.js` and invokes it.
   ```tsx
   const Counter_onClick = () => {
     const [count, props] = useLexicalScope();
     return (count.value += props.step || 1);
   ...
   ```
6. At this point, the execution is handed off from Unisynthloader to the lazy-loaded chunk. This is done so that the Unisynthloader can be as small as possible as it is inlined into the HTML.
7. `useLexicalScope` is imported from `@khulnasoft.com/unisynth` and is responsible for retrieving the `count` and `props`.
   `const [count, props] = useLexicalScope();`
8. Parse the `<script type="unisynth/json">{...json...}</script>` JSON and distribute the deserialized objects per `q:obj` attribute. In our case
...
9. Once the `unisynth/json` is deserialized, `useLexicalScope` can use the QRL's `[0,1]` array to look into `q:obj="456, 123"` to retrieve object with id `456` and `123`, which are props of `<div q:id="123" q:obj="456" q:host>` as well as `store` from `Counter_onMount` function.

> **NOTE:** For performance reasons the `q:obj` and `<script type="unisynth/json">` are only updated when the application is deserialized into HTML. When the application is running, these attributes may have stale values.
...
1. For Unisynth to work, the QRLs need to be serialized into HTML. This is problematic for dynamic `import()` because there is no easy way to retrieve the relative URL from the `import('./some-path.js')` so that it can be placed in the HTML.
...
Because of the above differences, Unisynth introduces QRLs as a mechanism for lazy loading closures into a Unisynth application.

## See Also

- [`qrl`: API used to create QRLs.](https://unisynth.dev/api/unisynth/#qrl)
