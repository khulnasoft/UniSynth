...
Unisynth provides a context API, which solves the problem of props drilling and it is very similar to React's functional `useContext()`. In fact, Unisynth's context API is the most efficient way to pass down data to different components, reducing overhead, generating less code, and allowing Unisynth to more effectively [treeshake](https://developer.mozilla.org/en-US/docs/Glossary/Tree_shaking) unused data.

Unisynth's context API is made of 3 methods, importable from `@khulnasoft.com/unisynth`:
...
export const UnisynthCityContext = createContextId<GenericType>(name: string): ContextId<GenericType>;
```

### Parameters

- `name`: is a unique string given to `createContextId` as an identifier of the context. This will avoid conflicts when there are multiple contexts. It is advised to use a naming convention like `io.builder.unisynth.city`.

### Returns

Notice that the value returned by `createContextId()` does not hold any state, it is an immutable ID object i.e. `{ id: 'io.builder.unisynth.city' }`. It's only used to describe the name and type of the context, like an address or an identifier.  Since it doesn't hold any state, it can be initialized as a singleton and exported from a shared module.
...
```tsx {9, 10, 11} /UnisynthCityContext/#a /PlainArrayContext/#b /AppNameContext/#c title="src/components/Parent.tsx"
import { component$, useStore, useContextProvider } from '@khulnasoft.com/unisynth';

export const Parent = component$(() => {

  const unisynthCityObject = useStore<GenericType>({
    ...
  });

  useContextProvider(UnisynthCityContext, unisynthCityObject);
  useContextProvider(PlainArrayContext, [1, 2, 3])
  useContextProvider(AppNameContext, "My Unisynth App")
...
- `data`: You can provide any data type, such as Unisynth's ﻿useSignal, ﻿useStore, arrays, or objects.
...
```tsx {4,5,6} /useContext/ /UnisynthCityContext/#a /PlainArrayContext/#b /AppNameContext/#c title="src/components/Children.tsx"
import { component$, useContext } from '@khulnasoft.com/unisynth';

export const Children = component$(() => {
  const unisynthCityObject = useContext(UnisynthCityContext);
