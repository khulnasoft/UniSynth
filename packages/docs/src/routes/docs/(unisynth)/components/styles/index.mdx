...
Unisynth does not enforce a specific styling approach, you can style your Unisynth app using any method you prefer, such as CSS, CSS-in-JS, CSS modules...

## CSS Modules

Unisynth supports [CSS Modules](https://github.com/css-modules/css-modules) out of the box thanks to [Vite](https://vitejs.dev/guide/features.html#css-modules).
...
Unisynth also accepts arrays, objects, or a combination of them to assign multiple classes:
...
> Automatically, Unisynth will try to inline this file in production mode if the amount of CSS is less than 10KB. If the file is larger than 10KB, it will be loaded as a separate file.

## CSS-in-JS

Unisynth has first-class CSS-in-JS support using [styled-vanilla-extract](https://github.com/wmertens/styled-vanilla-extract), which provides an extremely efficient css-in-js solution without any runtime!

```tsx title="style.css.ts"
import { style } from 'styled-vanilla-extract/unisynth';
...
<span q:slot="pnpm">
```shell
pnpm run unisynth add styled-vanilla-extract
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add styled-vanilla-extract
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add styled-vanilla-extract
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add styled-vanilla-extract
```
</span>
</PackageManagerTabs>
...
> **How about emotion or other CSS-in-JS libs?** While extremely popular, emotion and other CSS-in-JS libs are not the best choice for Unisynth. [They are not optimized for runtime performance and they don't have a good SSR streaming support](https://dev.to/srmagura/why-were-breaking-up-wiht-css-in-js-4g9b), leading to a degraded server and client performance.

## Styled-components

The styled-components library is a popular tool in React-land for writing CSS-in-JS. Thanks to the same [styled-vanilla-extract](https://github.com/wmertens/styled-vanilla-extract) plugin, you can write your styles with styled-components syntax in Unisynth with zero-runtime cost!
...
import { styled } from 'styled-vanilla-extract/unisynth';
...
To use scoped CSS, you can use the `useStylesScoped$()` hook exported from `@khulnasoft.com/unisynth`.
...
```tsx
import { useStylesScoped$, component$ } from '@khulnasoft.com/unisynth';
...
Component styles allow Unisynth to lazy load the style information for the component only when needed, which avoids double loading during SSR hydration.

```tsx
import { useStyles$, component$ } from '@khulnasoft.com/unisynth';
...
Thanks to [Vite](https://vitejs.dev/guide/features.html#css-pre-processors), Unisynth supports CSS preprocessors like Sass, Less, Stylus, and PostCSS.

There is no need to install Unisynth-specific plugins for them, but the corresponding pre-processor itself must be installed:
...
<span q:slot="pnpm">
```shell
pnpm run unisynth add tailwind
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add tailwind
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add tailwind
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add tailwind
```
</span>
</PackageManagerTabs>
...
<span q:slot="pnpm">
```shell
pnpm run unisynth add postcss
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add postcss
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add postcss
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add postcss
```
</span>
</PackageManagerTabs>
...
What is needed is to load the styles independently from the component. This is what [`useStyles$()`](/docs/(unisynth)/components/styles/index.mdx#usestyles) is for. There are two scenarios:
