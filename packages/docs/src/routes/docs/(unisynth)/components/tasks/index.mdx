...
> **Note**: For systems that use hydration, the execution of the application happens twice. Once on a server (SSR/SSG) and once on the browser (hydration). This is why many frameworks have "effects" which only execute on the browser. That means that the code that runs on the server is different than the code that runs on the browser. Unisynth execution is unified, meaning if the code has already been executed on the server, it does not re-execute it on the browser.

**In Unisynth, there are only 3 lifecycle stages:**
...
Additionally, this task can be reactive and will re-execute when **tracked** [state](/docs/(unisynth)/components/state/index.mdx) changes.
...
In Unisynth, there isn't a specific "mount" step like in some other frameworks. Instead, components just start up directly where they're needed, either on a web server or in your browser. 
...
One unique aspect of Unisynth, is that components are mounted only **once** across the server and client. This is a property of resumability. What this means is that if `useTask$` is executed during Server-Side Rendering (SSR), it will not run again in the browser because Unisynth does not perform hydration.
...
>
> On the browser:
> - The `useTask$()` does not have to run or be downloaded eagerly because Unisynth knows that the task is subscribed to the `text` signal from the server execution.
> - When the user types in the input box, the `text` signal changes. Unisynth knows that the `useTask$()` is subscribed to the `text` signal and it is at this time that the `useTask$()` closure is brought into the JavaScript VM to be executed.
>
> The `useTask$()`
>
...
> Sometimes it is required to only run code either in the server or in the client. This can be achieved by using the `isServer` and `isBrowser` booleans exported from `@khulnasoft.com/unisynth/build` as shown above.
...
> **Caution**: The `useVisibleTask$()` should be used as a last resort, because it eagerly executes code on the client. Unisynth, through [resumability](), goes out of its way to delay the execution of code on the client, and `useVisibleTask$()` is an escape hatch that should be used with caution. See [Best Practices](/docs/(unisynthcity)/guides/best-practices/index.mdx#dont-register-events-eagerly-with-usevisibletask) for more details.
