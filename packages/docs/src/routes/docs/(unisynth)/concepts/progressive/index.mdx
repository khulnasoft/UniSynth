...
This connects  to the [core principle](../../concepts/think-unisynth/index.mdx) of Unisynth which focuses on delaying **the loading** and execution of JavaScript for as long as possible. Unisynth needs to break up the application into many lazy loadable chunks to achieve that.
...
Unisynth's architecture takes full advantage of modern tooling to automate the problem of entry point generation. Developers can write components normally, while the Unisynth optimizer will split the components into chunks and download them when needed.
...
[Optimizer](../../advanced/optimizer/index.mdx) is a code transformation that extracts functions into top-level importable symbols, which allows the Unisynth runtime to lazy-load the JavaScript on an as-needed basis.

The Optimizer and Unisynth runtime work together to achieve the desired result of fine-grained lazy loading.
