---
title: RouteAction$ | UnisynthCity
description: Learn about actions in UnisynthCity, allowing form submissions, and performing side effects such as writing to a database or sending an email.
contributors:
  - manucorporat
  - cunzaizhuyi
  - forresst
...
Actions can be declared using `routeAction$()` or `globalAction$()` exported from `@khulnasoft.com/unisynth-city`.
...
The best way to call an action is using the `<Form/>` component exported in `@khulnasoft.com/unisynth-city`.
...
import { component$, type Signal, useSignal } from '@khulnasoft.com/unisynth';
...
Unisynth comes with built-in support for [Zod](https://zod.dev/), a TypeScript-first schema validation that can be used directly with actions, using the `zod$()` function.
...
Actions can be declared using the `routeAction$()` or `globalAction$()` exported from `@khulnasoft.com/unisynth-city`, the only difference between the two is that `routeAction$()` is scoped to a route, while `globalAction$()` is globally available across the whole app.
...
> If you want to manage common reusable routeAction$() it is essential that this function is re-exported from within 'layout.tsx' or 'index.tsx file of the existing route otherwise it will not run or throw exception. For more information [check this section](/docs/(unisynthcity)/re-exporting-loaders/index.mdx).
...
import { globalAction$ } from '@khulnasoft.com/unisynth-city';
