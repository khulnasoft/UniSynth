---
title: Advanced Routing | Unisynth City
description: Learn about advanced routing in Unisynth City, including 404 page handling, grouped layouts, named layouts, nested layouts, and plugin.ts files.
contributors:
  - manucorporat
  - adamdbradley
  - cunzaizhuyi
...
For any given route, Unisynth City can choose how to best handle the 404 response for the user, whether it's with the default 404 page, a custom 404 page, or a dynamically generated 404 page.

### Default 404 Page

Rather than showing a blank page, by default Unisynth City provides a generic 404 page for any route that isn't handled. The default 404 page is rendered as a fallback when a custom 404 page was not found. We recommend providing a custom 404 page for a better user experience. Including the common header and navigation in a custom 404 page would help the user find the page they're looking for.
...
Additionally, using Unisynth City's directory-based routing allows for custom 404 pages to be created at different paths. For example, if `src/routes/account/404.tsx` was also added to the structure, then the custom account 404 page would only be applied to the `/account/*` routes, while all other 404s would use the root `404.tsx` page.

> Note: During development and in preview mode, the custom 404 pages will not be rendered, but instead the default Unisynth City 404 page will show. However, when building the app for production, the custom 404 page will be statically generated as a static `404.html` file.
...
Unisynth City defines the convention that layouts are within `src/routes` and the filename starts with `layout`. That's why the default layout is named `layout.tsx`. A named layout also starts with `layout` followed by a dash `-` and a unique name, such as `layout-narrow.tsx`.
