---
title: Caching | Unisynth City
description: Learn how to handle caching in your Unisynth City app to keep your site as fast as possible.
contributors:
  - steve8708
  - harishkrishnan24
  - maiieul
...
Caching responses is critical for keeping your site as fast as possible, both for [pages](/docs/(unisynthcity)/pages/index.mdx) as well as [middleware](/docs/(unisynthcity)/middleware/index.mdx).

A good default is to use [stale-while-revalidate](https://web.dev/stale-while-revalidate/) caching for all responses. 

For instance, we can add an `onGet` export to our root [layout](/docs/(unisynthcity)/layout/index.mdx) (`src/routes/layout.tsx`) like so, to apply good caching defaults site-wide:
...
Any method that takes a [request event](https://unisynth.dev/docs/advanced/request-handling/#request-event) can call `request.cacheControl` to set the cache control headers for the response:
...
You can see the full [API reference](https://unisynth.dev/api/unisynth-city-middleware-request-handler/) of options you can pass to `request.cacheControl`.
