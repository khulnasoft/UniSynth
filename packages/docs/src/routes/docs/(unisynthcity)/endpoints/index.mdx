...
<CodeSandbox src="/src/routes/demo/unisynthcity/middleware/proxy/index.tsx">
```tsx
import type { RequestHandler } from '@khulnasoft.com/unisynth-city';
...
The [`server$`](/docs/(unisynthcity)/server$/index.mdx) functions are usually a better option when you need to run some code on the server and return a response back to the app. The `server$` is strongly typed and provides a more convenient API for returning data.
