...
description: Learn the best practices for Unisynth, a guide for understanding how to use Unisynth to its full potential.
contributors:
  - mhevery
  - the-r3aper7
  - manucorporat
...
Unisynth optimizer can better optimize the reactivity of the application if the operations are inlined in the template.
...
Every time Unisynth "reads" a signal/store value
...
In that case, you can add `// eslint-disable-next-line unisynth/no-use-visible-task` to the line before "useVisibleTask$" to remove the warning.

### Register DOM events with `useOn()`, `useOnWindow()`, or `useOnDocument()`

Unisynth allows to register event listeners in a declarative way, using the `useOn()` or using JSX.
...
Instead, use the `useOnDocument()` hook to register events on the `document` object, this way Unisynth will not execute any JS until the event is triggered.
...
> See: [useLocation() Docs](/docs/(unisynthcity)/api/index.mdx#uselocation)
