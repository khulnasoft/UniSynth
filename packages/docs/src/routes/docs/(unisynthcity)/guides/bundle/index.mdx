...
Symbols are individual lazy-loadable pieces in Unisynth. A symbol is created whenever you use the `__$(__)` in your source code.  
...
- On the other extreme of the slider, we have a separate chunk for each symbol. This is how Unisynth applications behave during development. Each symbol is in its own chunk.
...
The good news is that with Unisynth you are in full control of which symbol goes into which chunk. Normally breaking up your application for lazy loading requires the developer to write dynamic imports and re-factor the code. In Unisynth, all `$()` are potential lazy load locations, all that is needed is to inform the bundler how to distribute the symbols.

## `unisynthVite()` Plugin

`unisynthVite()` plugin inside your `vite.config.ts` controls the symbol distribution. Normally the `entryStrategy` is set to `smart` which allows the Unisynth to make heuristic guesses as to how the symbols should be lazy loaded. But it is possible to override the heuristic by providing `manual` configuration like this in `vite.config.ts` file:
...
Unisynth apps employ a service worker to ensure that bundles are prefetched into the browser's cache and that any user interaction would result in a cache hit, hence no delay in interaction.

See [Speculative Module Fetching](/docs/(unisynthcity)/advanced/speculative-module-fetching/index.mdx).
...
`qsymbol` event is fired every time Unisynth needs to resolve a symbol. Listening to this event can give you insight into when different symbols are loaded by your application. The information can then be used to better optimize your bundles by putting symbols that are needed together in the same bundle.
