---
title: Environment variables | Unisynth City
description: Unisynth apps can read environment variables in two main ways build-time variables and server-side variables.
contributors:
  - manucorporat
  - the-r3aper7
  - mrhoodz
...
UnisynthCity includes a few environment variables out of the box
...
The preferred method for accessing environment variables in UnisynthCity is via `import.meta.env.*` (client side) or through the Request object when run server side.
...
Unisynth City adaptors use the following:
