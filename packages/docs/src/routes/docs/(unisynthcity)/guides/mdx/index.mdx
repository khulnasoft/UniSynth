...
description: Learn how to use Markdown and MDX in Unisynth City.
contributors:
  - manucorporat
  - adamdbradley
  - Oyemade
...
An alternative way to author content is using `.mdx` files ([Markdown JSX](https://mdxjs.com/)). These files are authored as Markdown, but they are compiled down to Unisynth components. In addition to Markdown syntax, `.mdx` files can also refer to other components.
...
[MDX](https://mdxjs.com/) is a creative opportunity for you to come up with new content quickly ("Unisynthly" 🙂) and if you need more interaction on your page you can seamlessly integrate your Unisynth components like so:
...
**Note**: A key difference between Unisynth City and many current meta-frameworks is directory-based routing. Every route needs to be defined as `a-directory/index.(tsx,ts,js,jsx,md,mdx)`.

In other meta-frameworks you're used to `about.mdx` will render a route `http://example.com/about`. However, this will not work in Unisynth City. You must rename the file to `about/index.mdx` for Unisynth City to know to render it.

## Disabling default MDX plugins included.

Unisynth City includes 3 plugins by default.
