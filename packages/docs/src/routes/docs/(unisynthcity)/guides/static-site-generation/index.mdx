...
description: Learn how static site generation (SSG) works in Unisynth City.
contributors:
  - adamdbradley
  - AnthonyPAlicea
  - mhevery
...
Additionally, due to Unisynth's underlying architecture, page performance also benefits by not requiring a Javascript "hydration" step, which can significantly lower performance and slow down user interactivity. By pre-rendering static `index.html` files with SSG, and combined with Unisynth's [resumability](../../../(unisynth)/concepts/resumable/index.mdx), static site generation offers many performance benefits over legacy solutions.

## SSG vs. Server-Side Rendered (SSR)

Unisynth City is capable of taking a Unisynth application, no matter if it's a "webapp" or "website", and generate static HTML. Once it's generated as HTML, Unisynth is fundamentally able to skip rebuilding the app by using [resumability](../../../(unisynth)/concepts/resumable/index.mdx), since the app was already generated as HTML. Both Static Site Generation (SSG) and Server-Side Rendering (SSR) use the same process to generate the HTML. The main difference between the SSG and SSR however, is "when" the HTML is generated.
...
With Unisynth City however, the decision to use SSG or SSR, does not have to be one or the other decision. Instead, your own implementation can choose to have some route paths use SSG, while some other pages use SSR. It's entirely up to you and your requirements.
...
The URL `origin`, which is a combination of the scheme (protocol) and hostname (domain). For example, `https://unisynth.dev` has the protocol `https://` and domain `unisynth.dev`. However, the `origin` does _not_ include a `pathname`.
...
If the site also starts with a pathname other than `/`, please use the `base` option in the Vite config options (the `basePathname` option in the Unisynth City config options is deprecated).
...
For a Javascript project, it's quite common for the build's runtime to be built on top of [Node.js](https://nodejs.org/en/docs/). However, the core of Unisynth City static site generation isn't tied to using only Node.js, which is why the `unisynthCityGenerate()` function is imported from `@khulnasoft.com/unisynth-city/static/node`. By scoping the generate function to a specific runtime, such as Node.js, this gives Unisynth City the flexibility to also generate SSG from other runtimes in the future, such as [Deno](https://deno.land/) or [Bun](https://bun.sh/).
...
import { useLocation, type StaticGenerateHandler } from '@khulnasoft.com/unisynth-city';
...
Notice that the `index.tsx` file is inside a directory named `[id]`. This is a special directory name that tells Unisynth City to generate HTML files for each `id` param. The `index.tsx` file is the default file that Unisynth City will use when generating HTML files for a route path.
