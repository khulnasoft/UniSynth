---
title: Unisynth in a nutshell | Introduction
description: Learn the general concepts of Unisynth in this short introduction guide.
contributors:
  - manucorporat
  - AnthonyPAlicea
  - the-r3aper7
...
---
# Unisynth in a nutshell

## Components

Unisynth components are very similar to React components. They are functions that return JSX. However, `component$(...)` needs to be used, event handlers must have the `$` suffix, state is created using `useSignal()`, `class` is used instead of `className` and some other differences.


```tsx title="src/components/my-other-component/index.tsx"
import { component$, Slot } from '@khulnasoft.com/unisynth';
import type { ClassList } from '@khulnasoft.com/unisynth'
...
Content projection is done by the `<Slot/>` component, which is exported from `@khulnasoft.com/unisynth`. Slots can be named, and can be projected into using the `q:slot` attribute.
...
Methods that start with `use` are special in Unisynth, such as `useSignal()`, `useStore()`, `useOn()`, `useTask$()`, `useLocation()` and so on. Very similar to React hooks.
...
Unisynth supports out of the box, CSS modules, or even Tailwind, global CSS imports and lazy loading scoped CSS using `useStylesScoped$()`. CSS Modules are the recommended way to style Unisynth components.
...
Remember that Unisynth uses `class` instead of `className` for CSS classes.

## $(...) rules

The `$(...)` function and any function that end with `$` are special in Unisynth, such as: `$()`, `useTask$()`, `useVisibleTask$()`... The `$` at the end represents a lazy loading boundary. There are some rules that apply to the first argument of any `$` function. It is NOT related to jQuery at all.
...
Since Unisynth does NOT execute any Javascript on the browser before user interaction, `useVisibleTask$()` is the only API that will run eagerly on the client, which is why it's a good place to do things like:
...
You can create dynamic routes by adding a folder with `[param]` in the route path. For example, `src/routes/user/[id]/index.tsx` will create a route at `/user/:id/`. In order to access the route parameter, you can use the `useLocation` hook exported from `@khulnasoft.com/unisynth-city`.
...
To link to other routes, you can use the `Link` component exported from `@khulnasoft.com/unisynth-city`. The `Link` component takes all the properties of the `<a>` HTMLAnchorElement. The only difference is that it will use the Unisynth router to SPA navigate to the route instead of doing a full page navigation.
...
The recommended way to load data from the server is to use the `routeLoader$()` function exported from `@khulnasoft.com/unisynth-city`. The `routeLoader$()` function is used to create a data loader that will be executed on the server before the route is rendered. The return of `routeLoader$()` must be exported as a named export from the route file, ie, it can only be used in a `index.tsx`, inside of `src/routes/`.
...
Unisynth provides the `routeAction$()` API exported from `@khulnasoft.com/unisynth-city` to handle form requests on the server. `routeAction$()` is only executed on the server when the form is submitted.

```tsx title="src/routes/user/[userID]/index.tsx"

import { component$ } from '@khulnasoft.com/unisynth';
import { routeAction$, Form, zod$, z } from '@khulnasoft.com/unisynth-city';
...
The `routeAction$()` is used on pair with the `Form` component exported from `@khulnasoft.com/unisynth-city`. The `Form` component is a wrapper around the native HTML `<form>` element. The `Form` component takes an `ActionStore<T>` as the `action` property. The `ActionStore<T>` is the return value of the `routeAction$()` function.


## Run code only in the browser

Since Unisynth executes the same code on the server and in the browser, you cannot use `window` or other browser APIs in your code, because they don't exist when the code is executed on the server.
...
```tsx
import { component$, useTask$, useVisibleTask$, useSignal } from '@khulnasoft.com/unisynth';
...
Sometimes you need to run code only on the server, such as fetching data or accessing a database. To solve this problem, Unisynth provides a few APIs to run code only on the server.

```tsx
import { component$, useTask$ } from '@khulnasoft.com/unisynth';
import { server$, routeLoader$ } from '@khulnasoft.com/unisynth/unisynth-city';
...
Instead of `if(typeof window !== 'undefined')`, it is recommended to use the `isServer` and `isBrowser` boolean helpers exported from `@khulnasoft.com/unisynth/build` to ensure your code only runs in the browser. They contain slightly more robust checks to better detect the browser environment.
