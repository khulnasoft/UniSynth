---
title: HTML attributes | UnisynthCity
...
To apply these attributes in Unisynth city, add them to `containerAttributes` in the `src/entry.ssr.tsx` file:
...
export default function (opts: RenderToStreamOptions) {
  ...
  const isRTL = opts.serverData?.unisynthcity.params.locale === 'ar';
