---
title: Project Structure | Unisynth City
contributors:
  - manucorporat
  - steve8708
  - mhevery
...
A typical Unisynth project looks like this:
...
The `src/routes/` directory is a special directory where Unisynth City will look for your pages. Folders and files inside this directory have a special meaning and they will be mapped to the URL of your app.
...
Unisynth uses Vite to build the project. The `vite.config.ts` file contains the Vite configuration. It's standard for any Vite project. Please refer to the [Vite documentation](https://vitejs.dev/config/) for more information.

## Utilities

Unisynth has a utility command called `new` that allows developers to easily create new components and routes.
...
<span q:slot="pnpm">
```shell
pnpm run unisynth new Button
```
</span>
<span q:slot="npm">
```shell
npm run unisynth new Button
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth new Button
```
</span>
<span q:slot="bun">
```shell
bun run unisynth new Button
```
</span>
</PackageManagerTabs>
...
<span q:slot="pnpm">
```shell
pnpm run unisynth new /contact
```
</span>
<span q:slot="npm">
```shell
npm run unisynth new /contact
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth new /contact
```
</span>
<span q:slot="bun">
```shell
bun run unisynth new /contact
```
</span>
</PackageManagerTabs>

The following commands are consistent with Unisynth's directory file structure, allowing you to scaffold components more quickly.

If we compare our `unisynth-app-demo` from the top of the page, the additional changes would look like this:
...
<span q:slot="pnpm">
```shell
pnpm run unisynth new --barrel Button
```
</span>
<span q:slot="npm">
```shell
npm run unisynth new --barrel Button
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth new --barrel Button
```
</span>
<span q:slot="bun">
```shell
bun run unisynth new --barrel Button
```
</span>
</PackageManagerTabs>
...
> This feature was added in Unisynth v1.2, and those using an older version will not see this functionality.
