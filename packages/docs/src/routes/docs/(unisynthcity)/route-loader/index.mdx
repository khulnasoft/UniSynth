---
title: RouteLoader$ | Unisynth City
contributors:
  - manucorporat
  - mhevery
  - wtlin1228
...
Route Loaders load data in the server so it becomes available to use inside Unisynth Components. They trigger when SPA/MPA navigation happens so they can be invoked by Unisynth Components during rendering.

Please note that route loaders should be exported only from `layout.tsx` or `index.tsx` files. But they can be declared in any valid way ES modules allow. To reuse a route loader across multiple `layout.tsx` or `index.tsx files, define it in a separate file, export it, then import it in `layout.tsx` or `index.tsx files and [`re-export`](/docs/(unisynthcity)/re-exporting-loaders/index.mdx) it as a named export.

> If you want to manage common reusable routeLoaders$ it is essential that this function is re-exported from within 'layout.tsx' or 'index.tsx file of the existing route otherwise it will not run or throw exception. For more information [check this section](/docs/(unisynthcity)/re-exporting-loaders/index.mdx).
...
export default component$(() => {
  // In order to access the `routeLoader$` data within a Unisynth Component, you need to call the hook.
...
Multiple `routeLoader$`s are allowed across the whole application, and they can be used in any Unisynth Component. **You can even declare multiple `routeLoader$`s in the same file**.
...
Loaders execute after the Unisynth Middleware handlers (`onRequest`, `onGet`, `onPost`, etc), and before the Unisynth Components are rendered. This allows the loaders to start fetching data as soon as possible, reducing latency.
