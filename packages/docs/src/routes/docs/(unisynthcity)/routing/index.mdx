---
title: Routing | Unisynth City
description: Learn about routing in Unisynth City which is like other file-system based such as Next.js, SvelteKit, SolidStart or Remix.
contributors:
  - manucorporat
  - nnelgxorz
  - the-r3aper7
...
Inside the `src/routes` directory, all files named `index` are considered pages/endpoint/middleware, Unisynth supports the following extensions: `.ts`, `.tsx`, `.md` and `.mdx`.
...
When an `index.tsx` or `index.ts` file exports a Unisynth component as the default export, Unisynth City will render the component and return an HTML response as a webpage.
...
An `index.ts` file can access the HTTP request directly and return a raw HTTP response without involving any Unisynth Component. This is done by exporting any of the following methods: `onRequest`, `onGet`, `onPost`, `onPut` or `onDelete` depending on how you want to handle a specific HTTP request.
...
> Notice that in the last example, there is no default export. This is because you are not rendering a Unisynth component, but rather you are handling the request directly and returning a JSON response. This is useful for implementing RESTful APIs or any other type of HTTP endpoint.

### Page + Endpoint

In Unisynth City there is no clear separation between pages and endpoints. An `index.tsx` file handles both and exports a Unisynth component or an `onRequest` method. However, it's possible to combine both approaches. For example, you can export an `onRequest` method that will handle the request, and then render a Unisynth component.
...
Layout modules are very similar to `index` files. Both can handle requests and render Unisynth components. However, layouts are designed to work like middleware, allowing to **share UI and request handling (middleware)** to a set of routes.
...
For the given example, the Unisynth components will be rendered in the following order:
...
With Unisynth, the distinction between MPA and SPA disappears; every app can be both at the same time.
...
Unisynth provides a `<Link>` component and `useNavigate()` hook.
...
```tsx
import { component$ } from '@khulnasoft.com/unisynth';
import { Link, useNavigate } from '@khulnasoft.com/unisynth-city';
...
> The `Link` component uses the `useNavigate()` hook [internally](https://github.com/khulnasoft/unisynth/blob/e452582f4728cbcb7bf85d03293e757302286683/packages/unisynth-city/runtime/src/link-component.tsx#L33).
...
```tsx
import { component$ } from '@khulnasoft.com/unisynth';
import { Link, routeLoader$, useNavigate } from '@khulnasoft.com/unisynth-city';
...
Unisynth provides best-in-class scroll restoration for SPA's that closely mimics the native browser experience.
...
Unisynth will automatically restore their scroll position and bootstrap itself back into the SPA context as needed.

> The script required to provide this robust experience never loads, nor is it ever even sent to the user's browser,
> unless the history entry has had an SPA context. Pure MPA pages never load this script. This is the magic of Unisynth.

> Scroll restoration in Unisynth always occurs synchronously with render. When combined with Unisynth's resumable and
> first-class SSR/MPA nature, the user should never experience scroll flashing.

Unisynth's scroll restoration is entirely `history` based. This is different from many other frameworks
which rely on things like `sessionStorage`.

Unisynth's ability to remember and restore scroll positions is extremely robust
...
>
> On a page with an SPA context, Unisynth will patch the `pushState()` and `replaceState()` functions on the global `history`.
...
> This is because Unisynth needs to be able to automatically append the SPA context to the state as a property.
>
> If you provide a value that is not an object, Unisynth will create a new object for state and add your provided
> value to a new key: `{ _data: <your_value> }`
>
> Unisynth will also warn you in the browser's console in `dev` mode when this occurs.
...
Unisynth City also supports:

- [Route Parameters](/docs/(unisynthcity)/advanced/routing/index.mdx)
- [Nested layouts](/docs/(unisynthcity)/advanced/routing/index.mdx#nested-layout)
