...
> `server$` can accept any number of arguments and return any value that can be serialized by Unisynth. This includes primitives, objects, arrays, bigint, JSX nodes, and even Promises, just to name a few.
...
This API is actually used to implement UnisynthGPT streaming responses in our docs site.
