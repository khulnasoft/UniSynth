...
Unisynth is inspired by many modern frameworks. You'll see components that look like React components and use React-like hooks, but have simple mutable state like Vue, use a static form of JSX like Solid, compile away like Svelte, and use a simple, prescriptive structure like Angular.

An example of a Unisynth component showing several features:
...
Unisynth is component-driven like most modern frontend frameworks. Each Unisynth component should be in its own file and be the single default export. They are simple functions that return JSX elements
...
In general all imports inside a Unisynth component are passed to the generated output file.
...
> **Note:** The Unisynth cli will only move `.ts` and `.js` files to the output directory.
> You need to move the `.css` file by your own.

### `headless components`

Another approach is to develop your components without a fixed style inside the Unisynth component, a.k.a. "headless components".
...
You can then use `class` or `data-*` attributes inside your Unisynth component to apply those styles:
...
Unisynth prefers that you use `class` to provide class name strings, but it also allows you to provide `className`. If both are used in the same component, it will attempt to merge the two. We recommend that you only use one (preferrably `class`, as that's what is internally preferred by Unisynth).

## Control flow

Control flow in Khulnasoft is static like [Solid](https://github.com/ryansolid/solid). Instead of using freeform javascript like in React, you must use control flow components like `<Show>` and `<For>`
...
If the `Layout` component was also a Unisynth component then we simply use the reference in the props.
...
Unisynth compiles one component at a time and is only concerned with outputting the correct method for each framework. For the two examples above, here are the angular and html outputs.
