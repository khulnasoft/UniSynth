...
This Dockerfile is used to build a Docker image for our Unisynth Node.js application. You can edit it accordingly to use pnpm, npm, yarn or bun, by replacing yarn commands and `yarn.lock` file.
...
We finally specify the default command to run our Unisynth application with `yarn serve`, exposing the port 3000: Please note that port number should match the one you chose on your entry adapter (e.g.: `src/entry.express.tsx`).
