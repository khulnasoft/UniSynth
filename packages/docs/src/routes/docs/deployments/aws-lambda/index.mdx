...
Unisynth City AWS Adapter allows you to connect Unisynth City to [AWS Lambda](https://docs.aws.amazon.com/lambda/latest/dg/nodejs-handler.html?icmpid=docs_lambda_hel).
...
<span q:slot="pnpm">
```shell
pnpm run unisynth add aws-lambda
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add aws-lambda
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add aws-lambda
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add aws-lambda
```
</span>
</PackageManagerTabs>
