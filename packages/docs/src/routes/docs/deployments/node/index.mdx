...
Unisynth City Node middleware allows you to connect Unisynth City to a Node.js server which uses the common middleware functionality. Some Node servers include:
...
<span q:slot="pnpm">
```shell
pnpm run unisynth add express
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add express
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add express
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add express
```
</span>
</PackageManagerTabs>
...
<span q:slot="pnpm">
```shell
pnpm run unisynth add fastify
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add fastify
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add fastify
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add fastify
```
</span>
</PackageManagerTabs>
...
By default, all Unisynth City applications are protected against [CSRF attacks](https://owasp.org/www-community/attacks/csrf) for all POST, PATCH, DELETE form submits.
...
If you want to disable CSRF protection, you can set `checkOrigin: false` in the `createUnisynthCity()` options in `src/entry.preview.tsx` or `src/entry.[server].tsx`:
