...
Unisynth City Vercel Edge adapter allows you to connect Unisynth City to [Vercel Edge Functions](https://vercel.com/docs/concepts/functions/edge-functions).
...
<span q:slot="pnpm">
```shell
pnpm run unisynth add vercel-edge
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add vercel-edge
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add vercel-edge
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add vercel-edge
```
</span>
</PackageManagerTabs>
...
import { createUnisynthCity } from '@khulnasoft.com/unisynth-city/middleware/vercel-edge';
...
```tsx 
└── The Edge Function "_unisynth-city" is referencing unsupported modules
