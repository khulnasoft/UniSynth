...
You can add Auth.js easily by using the following Unisynth starter script:

<PackageManagerTabs>
<span q:slot="pnpm">
```shell
pnpm run unisynth add auth
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add auth
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add auth
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add auth
```
</span>
</PackageManagerTabs>
...
### Migrating from `@khulnasoft.com/unisynth-auth`

When migrating from `@khulnasoft.com/unisynth-auth` to `@auth/qwik`, there are a couple changes that need to be made.
...
2. The setup function and and methods exported by it have changed in `plugin@auth.ts`. Be sure to update the imports for the `useAuthSession`, `useAuthSignin`, and `useAuthSignout` methods anywhere you use them in your app.
    ...
    -import { serverAuth$ } from "@khulnasoft.com/unisynth-auth";
...
export default component$(() => {
  ...
  return (
    <Form action={signIn}>
      ...
      <input type="hidden" name="options.redirectTo" value="http://unisynth-auth-example.com/dashboard" />
...
export default component$(() => {
  const signIn = useSignIn();
  return (
    <button onClick$={() => signIn.submit({ providerId: 'github', options: { redirectTo: 'http://unisynth-auth-example.com/dashboard' } })}>Sign In</button>
  );
});
```
...
> *IMPORTANT*: Please read the Unisynth documentation about [Environment Variables](/docs/guides/env-variables/) to ensure you are using them safely. Many provider secrets should be kept secure and not exposed to the client/browser.
