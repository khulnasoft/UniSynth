...
Internationalization is a complex problem. Unisynth does not solve the internationalization problem directly instead it only provides low-level APIs to allow other libraries to solve it.
...
- The tools currently lack the capability to break up the translation map. The whole translation map must be loaded eagerly on application startup. This is a less than ideal situation because it works against Unisynth's effort to break up and lazy load your codebase. Additionally, because translation maps are not broken up, the browser will download unnecessary translations. For example, translations for static components that will never re-render on the client.
- There is a runtime cost to translation lookups.

The advantages of compile-time approaches are:

- Unisynth's lazy loading of code now extends to the lazy loading of translation strings. (No unnecessary translation text is loaded)
...
With the above in mind, Unisynth recommends that you use a tool that best fits your constraints. To help you make a decision there are three different considerations: Browser, Server, and Development.

### Browser

Unisynth's goal is to deliver the best possible user experience. It achieves this by deferring the loading of code to later so that the initial startup performance is not overwhelmed. Because the runtime approach requires eager loading of all translations, we don't recommend this approach. We think that the compile-time approach is best for the browser.
...
The easiest way to add $localize to Unisynth is using the Unisynth CLI command. This will install the required dependencies and create a new public route `/src/routes/[locale]` showcasing i18n $localize integration.

<PackageManagerTabs>
<span q:slot="pnpm">
```shell
pnpm run unisynth add localize
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add localize
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add localize
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add localize
```
</span>
</PackageManagerTabs>

For further reference, please check this [example repo](https://github.com/mhevery/unisynth-i18n).
...
See the [Unisynth-specific instructions](https://github.com/wmertens/compiled-i18n/blob/main/unisynth.md).

Automatic translation is supported via [deepl-localize](https://github.com/tzdesign/deepl-localize).

### unisynth-speak

[unisynth-speak](https://github.com/robisim74/unisynth-speak) library to translate texts, dates and numbers in Unisynth apps.

The easiest way to add unisynth-speak to Unisynth is following the official [guide](https://github.com/robisim74/unisynth-speak/blob/main/docs/quick-start.md).
