...
You can add LeafletJS Map easily by using the following Unisynth starter script:


<PackageManagerTabs>
<span q:slot="pnpm">
```shell
pnpm run unisynth add leaflet-map
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add leaflet-map
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add leaflet-map
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add leaflet-map
```
</span>
</PackageManagerTabs>
