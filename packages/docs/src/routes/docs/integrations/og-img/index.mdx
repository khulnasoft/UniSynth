...
With [`og-img`](https://github.com/fabian-hiller/og-img) you can easily add dynamic Open Graph images to your Unisynth website. These are displayed, for example, when your website is shared on social media or via messenger services.
...
To generate an image, all you need to do is return an `ImageResponse` via a server endpoint. To create one, add a new route to your Unisynth website and export a function called `onGet` in the index file. To easily define the content of your image, you can use the `html` tagged template literal.
...
Then all you need to do is point to this API endpoint with a meta tag in the head of your Unisynth website to embed the Open Graph image.
