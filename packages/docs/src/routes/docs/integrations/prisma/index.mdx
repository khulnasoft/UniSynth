...
Prisma can be used in Unisynth with `routeLoader$`, `routeAction$` and `server$` functions. These are Unisynth APIs to allow code to execute only on the server-side.

The easiest way to add Prisma to Unisynth is using the Unisynth CLI command. This will install the required dependencies and create a `prisma` folder with the Prisma schema and the migrations.


<PackageManagerTabs>
<span q:slot="pnpm">
```shell
pnpm run unisynth add prisma
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add prisma
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add prisma
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add prisma
```
</span>
</PackageManagerTabs>
