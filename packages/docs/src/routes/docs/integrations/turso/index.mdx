...
You can easily add Turso by using the following Unisynth starter script:


<PackageManagerTabs>
<span q:slot="pnpm">
```shell
pnpm run unisynth add turso
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add turso
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add turso
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add turso
```
</span>
</PackageManagerTabs>
...
## How to use Turso inside Unisynth

Import `tursoClient` inside your routes and initiate a database client instance
within Unisynth's server-side APIs that expose the `RequestEvent` object, such as
