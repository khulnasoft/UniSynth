...
[Vitest](https://vitest.dev/) is a blazing-fast unit test framework powered by Vite. Because Unisynth City uses [Vite](https://vitejs.dev/) by default, Vitest is our defacto unit test framework.

## Usage

You can add vitest easily by using the following Unisynth starter script:


<PackageManagerTabs>
<span q:slot="pnpm">
```shell
pnpm run unisynth add vitest
```
</span>
<span q:slot="npm">
```shell
npm run unisynth add vitest
```
</span>
<span q:slot="yarn">
```shell
yarn run unisynth add vitest
```
</span>
<span q:slot="bun">
```shell
bun run unisynth add vitest
```
</span>
</PackageManagerTabs>
...
If you are looking for an example for a Component with UnisynthCity checkout [UnisynthCityMockProvider](/docs/(unisynthcity)/api/index.mdx#unisynthcitymockprovider).
