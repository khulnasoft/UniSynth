---
title: "\U0001F9EA Unisynth Labs | Overview"
contributors:
  - mhevery
  - Craiqser
  - mrhoodz
...
# 🧪 Unisynth Labs Overview

Unisynth Labs is an incubator for ideas not yet ready for production. It's a place where we can publish our "work in progress" so the community can try it out and provide feedback, without any guarantees that the feature is stable or will make it into production. 
...
> **DISCLAIMER:** Unisynth Labs is a place to experiment, as such:
...
Each Unisynth Labs feature can roughly be thought of as going through these stages:
...
1. As an `experimental` flag.
  Some features are distributed under an experimental flag. This means that the feature is already part of the main package but is not enabled by default. To enable the feature you need to set the corresponding flag in the `unisynthVite` `experimental[]` array.

2. As a separate node package.
  Unisynth labs are distributed as a separate node package. Because Unisynth Labs is "work in progress" the node package is not published to NPM but instead as a github URL. The package is continually updated and so it will always contains the latest build. (You may read up on installing node packages [here](https://docs.npmjs.com/cli/v8/commands/npm-install).)

  <PackageManagerTabs>
  <span q:slot="pnpm">
  ```shell
  pnpm install github:khulnasoft/unisynth-labs-build#main
  ```
  </span>
  <span q:slot="npm">
  ```shell
  npm install github:khulnasoft/unisynth-labs-build#main
  ```
  </span>
  <span q:slot="yarn">
  ```shell
  yarn add github:khulnasoft/unisynth-labs-build#main
  ```
  </span>
  <span q:slot="bun">
  ```shell
  bun install github:khulnasoft/unisynth-labs-build#main
  ```
  </span>
  </PackageManagerTabs>
  ...
  ```javascript
  {
    ...
    "dependencies": {
      ...
      "@khulnasoft.com/unisynth-labs": "github:khulnasoft/unisynth-labs-build#main",
    }
  }
  ```
