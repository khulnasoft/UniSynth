---
title: "\U0001F9EA Insights | Unisynth Labs"
contributors:
  - mhevery
  - gioboa
  - mrhoodz
...
Insights allow your application to collect real user usage information to optimize the creation of bundles. By observing real user behavior, the Unisynth build system can then do a better job prefetching bundles for your application. There are two benefits of this:
...
- A `unisynthInsights` Vite Plugin to load and save real user usage data during the build process.

> NOTE:
> To try this new feature please drop a message inside the [Unisynth Discord server](https://unisynth.dev/chat)
...
You can get `PUBLIC_UNISYNTH_INSIGHTS_KEY` by visiting [Unisynth Insight](https://insights.unisynth.dev/app/add/).
...
export default defineConfig(async () => {
  return {
    plugins: [
      unisynthInsights({
        publicApiKey: loadEnv('', '.', '').PUBLIC_UNISYNTH_INSIGHTS_KEY,
      }),
      //...
    ],
    // ...
