...
Props must be serializable so that Unisynth can resume and render each component independently from other components on the page. This poses a problem if we wish to pass a callback to a child component. Callbacks are functions and functions are not directly serializable, but they are serializable through the `$()` by converting them to QRLs first.

## QRLs

Functions passing across serializable boundaries must be done through QRLs. QRLs are serialized forms of a function. (See [QRL](/docs/(unisynth)/advanced/qrl/index.mdx) in the advanced section.)

Unisynth has convenience APIs that end in `$` - these are equivalent to calling `$()` directly. These two lines are equivalent:
