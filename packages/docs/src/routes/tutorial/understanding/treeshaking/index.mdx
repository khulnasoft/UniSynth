...
A key concept of Unisynth is that Unisynth only loads code for components in the client that need to be re-rendered. If a component is static (does not need to be re-rendered), Unisynth will not load the component. This is a form of dynamic tree-shaking. Here the word dynamic is used to differentiate it from the classical static tree-shaking.
...
Unisynth determines which components are needed based on the runtime context of the application. Databinding determines whether the component is considered static or dynamic and, therefore, whether the component will be tree-shaken.
