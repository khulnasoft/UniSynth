...
const createRule = ESLintUtils.RuleCreator(
  (name) => `https://unisynth.dev/docs/advanced/eslint/#${name}`
);
...
export const validLexicalScope = createRule({
  ...
  meta: {
    ...
    messages: {
      referencesOutside:
        'When referencing "{{varName}}" inside a different scope ({{dollarName}}), Unisynth needs to serialize the value, however {{reason}}.\nCheck out https://unisynth.dev/docs/advanced/dollar/ for more details.',
      invalidJsxDollar:
...
function _isTypeCapturable(
  ...
  seen.add(type);
  if (type.getProperty('__no_serialize__') || type.getProperty('__unisynth_serializable__')) {
    return;
  }
...
export const validLexicalScopeExamples: UnisynthEslintExamples = {
  referencesOutside: {
    good: [
      {
