...
ruleTester.run('no-setter-with-same-name-as-state-prop', rule, {
  valid: [
    ...
    // Doesn't apply to none unisynth files
    {
      ...opts,
      code: `
      import { useStore } from "unisynth";
...
export default function MyComponent(props) {
  ...
  return (
    <div>
      ...
      `,
      errors: [
        'Cannot name a state property `setFoo` because of a collision with Unisynth-generated code for the state property `foo`. Please use a different name.',
      ],
    },
  ],
});
