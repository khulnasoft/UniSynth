import { configs as lintConfigs, rules } from '@khulnasoft.com/eslint-plugin-unisynth';
...
const TS_PARSER = 'unisynth-ts-parser';
...
Object.entries(lintConfigs.recommended.rules).forEach(([key, value]) => {
  const trimmedKey = key.replace(/^@khulnasoft.com\/unisynth\//, '');
