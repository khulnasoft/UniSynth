import {
    ...
    componentToUnisynth,
    componentToVue,
    khulnasoftContentToUnisynthComponent,
    mapStyles,
    parseJsx,
    UnisynthComponent,
...
const JSON_CODE_EXAMPLES = {
  ...
  SET_CONTEXT: {
    context: {
      set: {
        './KhulnasoftContext.context.lite:default': {
...
// Sync selections between the Khulnasoft editor and the fiddle
...
// TODO: Build this Fiddle app with Unisynth :)
export default function Fiddle() {
  const monaco = useMonaco();

  const [staticState] = useState(() => ({
    ignoreNextKhulnasoftUpdate: false,
  }));
  const [khulnasoftData, setKhulnasoftData] = useState<any>(null);
  const state = useLocalObservable(() => ({
    ...
    pendingKhulnasoftChange: null as any,
    khulnasoftData: {} as any,
    isDraggingKhulnasoftCodeBar: false,
    isDraggingJSXCodeBar: false,
    jsxCodeTabWidth: Number(localStorageGet('jsxCodeTabWidth')) || 45,
    khulnasoftPaneHeight: Number(localStorageGet('khulnasoftPaneHeight')) || 35,
      ...
      if (editor) {
        if (SYNC_SELECTIONS) {
          editor.onDidChangeCursorPosition((event) => {
            ...
            (
              document.querySelector('khulnasoft-editor iframe') as HTMLIFrameElement
            )?.contentWindow?.postMessage(
              {
                type: 'khulnasoft.changeSelection',
                data: {
                  index: elementIndex,
                },
              },
    ...
    applyPendingKhulnasoftChange(update?: any) {
      const khulnasoftJson = update || state.pendingKhulnasoftChange;
      if (!khulnasoftJson) {
        return;
      }
      const jsxJson = khulnasoftContentToUnisynthComponent(khulnasoftJson);
      state.code = componentToUnisynth()({ component: jsxJson });
      state.pendingKhulnasoftChange = null;
    },

    async updateOutput() {
      try {
        state.pendingKhulnasoftChange = null;
        staticState.ignoreNextKhulnasoftUpdate = true;

        let json: UnisynthComponent;
        ...
        const newKhulnasoftData = componentToKhulnasoft()({ component: json });
        setKhulnasoftData(newKhulnasoftData);
      } catch (err) {
        if (debug) {
          throw err;
        } else {
  ...
  useEventListener<KeyboardEvent>(document.body, 'keydown', (e) => {
    ...
    if ((e.metaKey || e.ctrlKey) && e.key === 's') {
      e.preventDefault();
      state.applyPendingKhulnasoftChange();
    }
  });
  ...
  useEventListener<MouseEvent>(document.body, 'mousemove', (e) => {
    if (state.isDraggingJSXCodeBar) {
    ...
    } else if (state.isDraggingKhulnasoftCodeBar) {
      ...
      state.khulnasoftPaneHeight = Math.min(newHeight, 95);
    }
  });

  useEventListener<MouseEvent>(document.body, 'mouseup', (e) => {
    state.isDraggingJSXCodeBar = false;
    state.isDraggingKhulnasoftCodeBar = false;
  });
  useEventListener<MessageEvent>(window, 'message', (e) => {
    if (e.data?.type === 'khulnasoft.saveCommand') {
      if (e.data.data || state.pendingKhulnasoftChange) {
        state.applyPendingKhulnasoftChange(e.data.data || state.pendingKhulnasoftChange);
      }
    } else if (e.data?.type === 'khulnasoft.selectionChange') {
      if (SYNC_SELECTIONS) {
  ...
  useReaction(
    () => state.khulnasoftPaneHeight,
    (width) => localStorageSet('khulnasoftPaneHeight', width),
    { fireImmediately: false, delay: 1000 },
  );
