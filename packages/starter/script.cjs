...
const main = async () => {
  p.intro(`Welcome to Unisynth! Let's get started.`);

  projectName = await p.text({
    message: 'What is your Unisynth project name?',
   ...
   */
  const updateUnisynthConfig = (filePath) => {
...
module.exports = ${JSON.stringify(config, null, 2)}`;
  ...
  const copy = (src, dest) => {
    ...
    fs.readdirSync(src).forEach((entry) => {
      ...
      // update unisynth.config.cjs
      if (entry === 'unisynth.config.cjs') {
        try {
          const newConfig = updateUnisynthConfig(srcPath);

          fs.writeFileSync(destPath, newConfig);
        } catch (error) {
          p.note('Error processing `unisynth.config.cjs`. Copying as-is.');
          console.error(error);
