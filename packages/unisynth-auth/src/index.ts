...
import { implicit$FirstArg, type QRL } from '@khulnasoft.com/unisynth';
import {
  globalAction$,
  routeLoader$,
  z,
...
export type UnisynthAuthConfig = AuthConfig;
...
export function serverAuthQrl(authOptions: QRL<(ev: RequestEventCommon) => UnisynthAuthConfig>) {
  const useAuthSignin = globalAction$(
    async ({ providerId, callbackUrl: deprecated, options, authorizationParams }, req) => {
      ...
      // set authjs.callback-url cookie. Fix for https://github.com/khulnasoft/unisynth/issues/5227
...
async function authAction(
  ...
  path: string,
  authOptions: UnisynthAuthConfig
) {
...
const patchAuthOptions = async (
  authOptions: QRL<(ev: RequestEventCommon) => UnisynthAuthConfig>,
  req: RequestEventCommon
) => {
