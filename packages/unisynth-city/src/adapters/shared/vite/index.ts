...
export function viteAdapter(opts: ViteAdapterPluginOptions) {
  let unisynthCityPlugin: UnisynthCityPlugin | null = null;
  let unisynthVitePlugin: UnisynthVitePlugin | null = null;
  let serverOutDir: string | null = null;
  let renderModulePath: string | null = null;
  let unisynthCityPlanModulePath: string | null = null;
  let isSsrBuild = false;
  let format = 'esm';
  const outputEntries: string[] = [];

  const plugin: Plugin<never> = {
    name: `vite-plugin-unisynth-city-${opts.name}`,
    enforce: 'post',
    apply: 'build',
    ...
    configResolved(config) {
      ...
      if (isSsrBuild) {
        unisynthCityPlugin = config.plugins.find(
          (p) => p.name === 'vite-plugin-unisynth-city'
        ) as UnisynthCityPlugin;
        if (!unisynthCityPlugin) {
          throw new Error('Missing vite-plugin-unisynth-city');
        }
        unisynthVitePlugin = config.plugins.find(
          (p) => p.name === 'vite-plugin-unisynth'
        ) as UnisynthVitePlugin;
        if (!unisynthVitePlugin) {
      ...
      if (isSsrBuild) {
        ...
        for (const fileName in bundles) {
          ...
          if (chunk.type === 'chunk' && chunk.isEntry) {
            ...
            } else if (chunk.name === '@unisynth-city-plan') {
              unisynthCityPlanModulePath = join(serverOutDir!, fileName);
            }
          }
        }
        ...
        if (!unisynthCityPlanModulePath) {
          throw new Error(
            'Unable to find "@unisynth-city-plan" entry point. Did you forget to add it to "build.rollupOptions.input"?'
          );
        }
      }
    },
    ...
    closeBundle: {
      ...
      async handler() {
        if (
          ...
          opts.ssg !== null &&
          serverOutDir &&
          unisynthCityPlugin?.api &&
          unisynthVitePlugin?.api
        ) {
          const staticPaths: string[] = opts.staticPaths || [];
          const routes = unisynthCityPlugin.api.getRoutes();
          const basePathname = unisynthCityPlugin.api.getBasePathname();
          const clientOutDir = unisynthVitePlugin.api.getClientOutDir()!;
          const clientPublicOutDir = unisynthVitePlugin.api.getClientPublicOutDir()!;
          ...
          if (renderModulePath && unisynthCityPlanModulePath && clientOutDir && clientPublicOutDir) {
            ...
            try {
              ...
              this.warn(
                `Invalid "origin" option: "${ssgOrigin}". Using default origin: "https://yoursite.unisynth.dev"`
              );
              ssgOrigin = `https://yoursite.unisynth.dev`;
            }
...
export interface AdapterSSGOptions extends Omit<StaticGenerateRenderOptions, 'outDir' | 'origin'> {
   ...
   * the Unisynth City config options.
   */
  origin?: string;
}

/** @public */
export const STATIC_PATHS_ID = '@unisynth-city-static-paths';

/** @public */
export const RESOLVED_STATIC_PATHS_ID = `${STATIC_PATHS_ID}.js`;

/** @public */
export const NOT_FOUND_PATHS_ID = '@unisynth-city-not-found-paths';
