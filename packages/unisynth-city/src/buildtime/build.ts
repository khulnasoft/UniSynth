...
function validateBuild(ctx: BuildContext) {
  ...
  ctx.layouts
    ...
    .forEach((l) => {
      addWarning(
        ctx,
        `The "top" layout feature, which is used by "${l.filePath}" has been deprecated and will be removed from future versions. In most cases the "group" layout feature can be used in its place: https://unisynth.dev/unisynthcity/layout/grouped/`
      );
    });
}
