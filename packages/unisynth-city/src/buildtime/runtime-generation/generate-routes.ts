import type { UnisynthVitePlugin, UnisynthManifest } from '@khulnasoft.com/unisynth/optimizer';
...
export function createRoutes(
  ...
  if (ctx.layouts.length > 0) {
    c.push(`\n/** Unisynth City Layouts (${ctx.layouts.length}) */`);
  ...
  c.push(`\n/** Unisynth City Routes (${ctx.routes.length}) */`);
  ...
  for (const route of ctx.routes) {
    ...
    if (loaders.length > 0) {
      c.push(`  ${createRouteData(unisynthPlugin, route, loaders, isSSR)},`);
    }
  }
...
function createRouteData(
  ...
  if (isSsr) {
    ...
    const clientBundleNames = JSON.stringify(getClientRouteBundleNames(unisynthPlugin, r));
...
function getClientRouteBundleNames(unisynthPlugin: UnisynthVitePlugin, r: BuildRoute) {
