import type { InsightManifest, UnisynthManifest } from '@khulnasoft.com/unisynth/optimizer';
...
export function prependManifestToServiceWorker(
  ...
  const key = `/* Unisynth Service Worker */`;
...
export function generateAppBundles(appBundles: AppBundle[], manifest: UnisynthManifest) {
...
function generateLibraryBundles(appBundles: AppBundle[], manifest: UnisynthManifest) {
  const libraryBundleIds: number[] = [];

  for (const [bundleName, bundle] of Object.entries(manifest.bundles)) {
    if (bundle.origins && bundle.origins.includes('@unisynth-city-plan')) {
