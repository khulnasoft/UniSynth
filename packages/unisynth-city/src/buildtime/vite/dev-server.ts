import type { UnisynthManifest, UnisynthViteDevResponse } from '@khulnasoft.com/unisynth/optimizer';
...
import { getUnisynthCityServerData } from '../../middleware/request-handler/response-page';
import {
      ...
      try {
        ...
        const renderFn = async (requestEv: RequestEvent) => {
          ...
          if (!isPageDataReq) {
            ...
            (res as UnisynthViteDevResponse)._unisynthEnvData = {
              ...(res as UnisynthViteDevResponse)._unisynthEnvData,
              ...serverData,
            };

            const unisynthRenderPromise = new Promise<void>((resolve) => {
              (res as UnisynthViteDevResponse)._unisynthRenderResolve = resolve;
            });

            next();

            return unisynthRenderPromise;
          }
        };
        ...
        if (requestHandlers.length > 0) {
          ...
          const { _deserializeData, _serializeData, _verifySerializable } =
            await server.ssrLoadModule('@unisynth-serializer');
          const unisynthSerializer = { _deserializeData, _serializeData, _verifySerializable };

          const { completion, requestEv } = runUnisynthCity(
            serverRequestEv,
            loadedRoute,
            requestHandlers,
            manifest,
  ...
  if (requestHandler?.name) {
    ...
    if (filePath) {
      try {
        ...
        err.plugin = 'vite-plugin-unisynth-city';
...
function shouldSkipRequest(pathname: string) {
  if (pathname.startsWith('/@unisynth-city-')) {
    return true;
  }
  if (
...
const DEV_SERVICE_WORKER = `/* Unisynth City Dev Service Worker */
