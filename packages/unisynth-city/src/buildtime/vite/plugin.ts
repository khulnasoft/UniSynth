import swRegister from '@unisynth-city-sw-register-build';
...
import { generateUnisynthCityPlan } from '../runtime-generation/generate-unisynth-city-plan';
...
import type { UnisynthCityPluginApi, UnisynthCityVitePluginOptions } from './types';
import { build } from '../build';
import { ssrDevMiddleware, staticDistMiddleware } from './dev-server';
import { transformMenu } from '../markdown/menu';
import { generateUnisynthCityEntries } from '../runtime-generation/generate-entries';
...
/** @public */
export function unisynthCity(userOpts?: UnisynthCityVitePluginOptions): PluginOption[] {
  return [unisynthCityPlugin(userOpts), ...imagePlugin(userOpts)];
}

function unisynthCityPlugin(userOpts?: UnisynthCityVitePluginOptions): any {
  let ctx: BuildContext | null = null;
  let mdxTransform: MdxTransform | null = null;
  let rootDir: string | null = null;
  let unisynthPlugin: UnisynthVitePlugin | null;
  ...
  globalThis.__unisynthCityNew = true;

  const api: UnisynthCityPluginApi = {
  ...
  const plugin: P<UnisynthCityPluginApi> = {
    name: 'vite-plugin-unisynth-city',
    enforce: 'pre',
    api,
    ...
    configureServer(server) {
      return () => {
        ...
        // unisynth city middleware injected BEFORE vite internal middlewares
        // and BEFORE @khulnasoft.com/unisynth/optimizer/vite middlewares
    ...
    resolveId(id) {
      if (id === UNISYNTH_SERIALIZER) {
        return join(rootDir!, id);
      }
      if (id === UNISYNTH_CITY_PLAN_ID || id === UNISYNTH_CITY_ENTRIES_ID) {
        return {
          id: join(rootDir!, id),
          // user entries added in the routes, like src/routes/service-worker.ts
          // are added as dynamic imports to the unisynth-city-plan as a way to create
          // a new entry point for the build. Ensure these are not treeshaked.
          moduleSideEffects: 'no-treeshake',
        };
      }
      if (id === UNISYNTH_CITY_SW_REGISTER) {
        return join(rootDir!, id);
      }
    ...
    async load(id, opts) {
      if (ctx) {
        if (id.endsWith(UNISYNTH_CITY_ENTRIES_ID)) {
          // @unisynth-city-entries
          return generateUnisynthCityEntries(ctx);
        }
        const isSerializer = id.endsWith(UNISYNTH_SERIALIZER);
        const isCityPlan = id.endsWith(UNISYNTH_CITY_PLAN_ID);
        const isSwRegister = id.endsWith(UNISYNTH_CITY_SW_REGISTER);

        if (isSerializer) {
          return `export {_deserializeData, _serializeData, _verifySerializable} from '@khulnasoft.com/unisynth'`;
        }
        if (isCityPlan || isSwRegister) {
          ...
          if (isCityPlan) {
            // @unisynth-city-plan
            return generateUnisynthCityPlan(ctx, unisynthPlugin!, opts?.ssr ?? false);
          }
    ...
    closeBundle: {
      ...
      async handler() {
        if (ctx?.target === 'ssr' && !ctx?.isDevServer) {
          // ssr build
          const manifest = unisynthPlugin!.api.getManifest();
          const clientOutDir = unisynthPlugin!.api.getClientOutDir();
          ...
          if (manifest && clientOutDir) {
            ...
            const insightsManifest = await unisynthPlugin!.api.getInsightsManifest(clientOutDir);
          ...
          if (outDir && clientOutDir) {
            const assetsDir = unisynthPlugin!.api.getAssetsDir();
...
const UNISYNTH_SERIALIZER = '@unisynth-serializer';
const UNISYNTH_CITY_PLAN_ID = '@unisynth-city-plan';
const UNISYNTH_CITY_ENTRIES_ID = '@unisynth-city-entries';
const UNISYNTH_CITY = '@khulnasoft.com/unisynth-city';
const UNISYNTH_CITY_SW_REGISTER = '@unisynth-city-sw-register';
