...
import type { ValueOrPromise } from '@khulnasoft.com/unisynth';
...
export const RequestEvUnisynthSerializer = Symbol('RequestEvUnisynthSerializer');
...
export function createRequestEvent(
  ...
  const requestEv: RequestEventInternal = {
    ...
    [RequestEvUnisynthSerializer]: unisynthSerializer,
    cookie,
    headers,
    env,
    ...
    parseBody: async () => {
      ...
      return (requestData = parseRequest(requestEv, sharedMap, unisynthSerializer));
    },
...
const parseRequest = async (
  ...
  if (type === 'application/x-www-form-urlencoded' || type === 'multipart/form-data') {
    ...
    return data;
  } else if (type === 'application/unisynth-json') {
    if (method === 'GET' && query.has(QDATA_KEY)) {
      const data = query.get(QDATA_KEY);
      if (data) {
        try {
          return unisynthSerializer._deserializeData(decodeURIComponent(data));
        } catch (err) {
          //
        }
      }
    }
    return unisynthSerializer._deserializeData(await request.text());
  }
  return undefined;
};
