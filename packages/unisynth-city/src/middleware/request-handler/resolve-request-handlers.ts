import type { QRL } from '@khulnasoft.com/unisynth';
import type { Render, RenderToStringResult } from '@khulnasoft.com/unisynth/server';
...
import { getUnisynthCityServerData } from './response-page';
import type { UnisynthSerializer, RequestEvent, RequestEventBase, RequestHandler } from './types';
...
export function actionsMiddleware(routeActions: ActionInternal[], routeLoaders: LoaderInternal[]) {
  return async (requestEv: RequestEventInternal) => {
    ...
    const unisynthSerializer = requestEv[RequestEvUnisynthSerializer];
    if (isDev && method === 'GET') {
      if (requestEv.query.has(QACTION_KEY)) {
        console.warn(
          'Seems like you are submitting a Unisynth Action via GET request. Unisynth Actions should be submitted via POST request.\nMake sure your <form> has method="POST" attribute, like this: <form method="POST">'
        );
      }
    }
    if (method === 'POST') {
      ...
      if (selectedActionId) {
        const serverActionsMap = globalThis._unisynthActionsMap as
        ...
        if (action) {
          ...
          if (!result.success) {
            ...
            if (isDev) {
              verifySerializable(unisynthSerializer, actionResolved, action.__qrl);
            }
        ...
        loaders[loaderId] = runValidators(
          ...
          .then((resolvedLoader) => {
            if (typeof resolvedLoader === 'function') {
              ...
              if (isDev) {
                verifySerializable(unisynthSerializer, resolvedLoader, loader.__qrl);
              }
              loaders[loaderId] = resolvedLoader;
            }
...
async function pureServerFunction(ev: RequestEvent) {
  ...
  if (
    ...
    ev.request.headers.get('Content-Type') === 'application/unisynth-json'
  ) {
    ev.exit();
    const isDev = getRequestMode(ev) === 'dev';
    const unisynthSerializer = (ev as RequestEventInternal)[RequestEvUnisynthSerializer];
      ...
      if (isQrl(qrl) && qrl.getHash() === fn) {
        ...
        try {
          ...
          if (err instanceof ServerError) {
            ev.headers.set('Content-Type', 'application/unisynth-json');
            ev.send(err.status, await unisynthSerializer._serializeData(err.data, true));
            return;
          }
          ev.headers.set('Content-Type', 'application/unisynth-json');
          ev.send(500, await unisynthSerializer._serializeData(err, true));
          return;
        }
        if (isAsyncIterator(result)) {
          ev.headers.set('Content-Type', 'text/unisynth-json-stream');
          const writable = ev.getWritableStream();
          const stream = writable.getWriter();
          for await (const item of result) {
            if (isDev) {
              verifySerializable(unisynthSerializer, item, qrl);
            }
            const message = await unisynthSerializer._serializeData(item, true);
          ...
          verifySerializable(unisynthSerializer, result, qrl);
          ev.headers.set('Content-Type', 'application/unisynth-json');
          const message = await unisynthSerializer._serializeData(result, true);
          ev.send(200, message);
        }
        return;
      }
...
export function verifySerializable(unisynthSerializer: UnisynthSerializer, data: any, qrl: QRL) {
  try {
    unisynthSerializer._verifySerializable(data, undefined);
  } catch (e: any) {
...
export function renderUnisynthMiddleware(render: Render) {
  return async (requestEv: RequestEvent) => {
    ...
    try {
      ...
      const serverData = getUnisynthCityServerData(requestEv);
...
export async function renderQData(requestEv: RequestEvent) {
  ...
  const unisynthSerializer = (requestEv as RequestEventInternal)[RequestEvUnisynthSerializer];
  // write just the page json data to the response body
  const data = await unisynthSerializer._serializeData(qData, true);
