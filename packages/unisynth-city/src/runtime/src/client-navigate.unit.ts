...
test('initialize and push empty scroll history state on navigate', () => {
  const [win, urlOf] = createTestWindow('http://unisynth.dev/');
  ...
  // This will be null, upgrading state only happens in UnisynthCityProvider.
...
test('pushState for different routes', () => {
  const [win, urlOf] = createTestWindow('http://unisynth.dev/page-a?search=123');
  assert.equal(win.history.state, null);
...
test('pushState for different hash', () => {
  const [win, urlOf] = createTestWindow('http://unisynth.dev/page-a?search=123#hash-1');
  assert.equal(win.history.state, null);
