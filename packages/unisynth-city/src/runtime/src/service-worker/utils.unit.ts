...
test('getCacheToDelete, delete bundles no longer possible', () => {
  ...
  const cachedUrls = ['https://unisynth.dev/build/q-abc.js', 'https://unisynth.dev/build/q-xyz.js'];
  const c = getCacheToDelete(appBundles, cachedUrls);
  assert.deepEqual(c, ['https://unisynth.dev/build/q-xyz.js']);
});
...
test('getCacheToDelete, none to delete', () => {
  ...
  const cachedUrls = ['https://unisynth.dev/build/q-abc.js'];
  const c = getCacheToDelete(appBundles, cachedUrls);
  assert.deepEqual(c, []);
});
