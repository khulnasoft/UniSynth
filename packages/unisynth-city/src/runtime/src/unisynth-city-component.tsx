...
import { isBrowser, isDev, isServer } from '@khulnasoft.com/unisynth/build';
...
import { useUnisynthCityEnv } from './use-functions';
...
/** @public */
export const UNISYNTH_CITY_SCROLLER = '_qCityScroller';

/** @public */
export interface UnisynthCityProps {
  // /**
  //  * The UnisynthCity component must have only two direct children: `<head>` and `<body>`, like the following example:
  //  *
  //  * ```tsx
...
/** @public */
export const UnisynthCityProvider = component$<UnisynthCityProps>((props) => {
  useStyles$(`:root{view-transition-name:none}`);
  const env = useUnisynthCityEnv();
  if (!env?.params) {
    throw new Error(
      `Missing Unisynth City Env Data for help visit https://github.com/khulnasoft/unisynth/issues/6237`
    );
  }

  const urlEnv = useServerData<string>('url');
  if (!urlEnv) {
    throw new Error(`Missing Unisynth URL Env Data`);
  }
  ...
  const goto: RouteNavigate = $(async (path, opt) => {
    ...
    if (!forceReload && isSamePath(dest, lastDest)) {
      ...
      // - Hashes in Link are already broken in Unisynth (<=v1.1.5), and <a> tags are untracked. (not a new bug)
    ...
    if (isBrowser) {
      ...
      loadRoute(unisynthCity.routes, unisynthCity.menus, unisynthCity.cacheModules, dest.pathname);
    }
    ...
    async function run() {
      ...
      if (isServer) {
        ...
        if (trackUrl.pathname.endsWith('/')) {
          if (!unisynthCity.trailingSlash) {
            trackUrl.pathname = trackUrl.pathname.slice(0, -1);
          }
        } else if (unisynthCity.trailingSlash) {
          trackUrl.pathname += '/';
        }
            ...
            if (!win._qCityHistoryPatch) {
              ...
              const prepareState = (state: any) => {
                if (state === null || typeof state === 'undefined') {
                  ...
                  if (isDev) {
                    console.warn(
                      'In a Unisynth SPA context, `history.state` is used to store scroll state. ' +
...
/** @public */
export const UnisynthCityMockProvider = component$<UnisynthCityMockProps>((props) => {
  ...
  const goto: RouteNavigate =
    props.goto ??
    $(async () => {
      console.warn('UnisynthCityMockProvider: goto not provided');
    });
