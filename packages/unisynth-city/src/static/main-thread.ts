...
export async function mainThread(sys: System) {
  ...
  const routes = unisynthCityPlan.routes || [];
  const trailingSlash = !!unisynthCityPlan.trailingSlash;
  ...
  return new Promise<StaticGenerateResult>((resolve, reject) => {
    try {
      ...
      const addToQueue = (pathname: string | undefined | null, params: PathParams | undefined) => {
        if (pathname) {
          pathname = new URL(pathname, `https://unisynth.dev`).pathname;
...
function validateOptions(opts: StaticGenerateOptions) {
  if (!opts.unisynthCityPlanModulePath) {
    throw new Error(`Missing "unisynthCityPlanModulePath" option`);
  }
