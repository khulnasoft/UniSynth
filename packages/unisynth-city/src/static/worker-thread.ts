...
import type { ServerRequestEvent } from '@khulnasoft.com/unisynth-city/middleware/request-handler';
...
async function workerRender(
  ...
  try {
    ...
    const promise = requestHandler(requestCtx, opts, unisynthSerializer)
      .then((rsp) => {
        if (rsp != null) {
