...
export function runUnisynthJsonDebug(window: Window, document: Document, debug: typeof unisynthJsonDebug) {
  const parseUnisynthJSON = () => {
    ...
    (window as any).unisynthJson = debugData;
    console.log(debugData);
  };
  if (document.querySelector('script[type="unisynth/json"]')) {
    parseUnisynthJSON();
  } else {
    document.addEventListener('DOMContentLoaded', parseUnisynthJSON);
  }
}

export function unisynthJsonDebug(
  document: Document,
  unisynthJson: UnisynthJson,
  derivedFns: Function[]
): DebugState {
  class Base {
    constructor(
  ...
  const objs: any[] = [];
  const subs: any[] = [];
  unisynthJson.objs.forEach((_, idx) => getObject(idx, null));
  Object.keys(unisynthJson.ctx).forEach((idx) => getContext(idx));
  Object.keys(unisynthJson.refs).forEach((idx) => getRef(idx));
  unisynthJson.subs;
  return {
    refs,
    ctx: contexts,
  ...
  function getObject(idx: number | string, parent: any): any {
    ...
    if (!obj) {
      const rawValue = unisynthJson.objs[idx];
...
export interface UnisynthJson {
  ...
  >;
  objs: Array<UnisynthJsonObjsPrimitives | UnisynthJsonObjsObj>;
  subs: Array<Array<string>>;
}
export type UnisynthJsonObjsPrimitives = string | boolean | number | null;
export type UnisynthJsonObjsObj = Record<string, UnisynthJsonObjsPrimitives>;
