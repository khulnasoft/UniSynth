...
interface UnisynthSymbolTrackerWindow extends Window {
  qSymbolTracker: {
    symbols: InsightSymbol[];
    publicApiKey: string;
  };
...
// the unisynth optimizer only minifies somewhat, so put all var declarations in the same line
const insightsPing = sync$(() =>
  ((
    window: UnisynthSymbolTrackerWindow,
    document,
    location,
    navigator,
    performance,
  ...
  ({ publicApiKey, postUrl }) => {
    ...
    return (
      ...
      <script
        ...
        dangerouslySetInnerHTML={`__QI_KEY__=${JSON.stringify(publicApiKey)};__QI_URL__=${JSON.stringify(postUrl || `https://insights.unisynth.dev/api/v1/${publicApiKey}/post/`)}`}
      />
    );
  }
);
