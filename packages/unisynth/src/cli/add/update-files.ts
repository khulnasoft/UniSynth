...
async function mergePackageJsons(fileUpdates: FsUpdates, srcPath: string, destPath: string) {
  ...
  try {
    ...
    if (destPkgJson.scripts?.unisynth) {
      const unisynthVal = destPkgJson.scripts.unisynth;
      delete destPkgJson.scripts.unisynth;
      destPkgJson.scripts.unisynth = unisynthVal;
    }
    fileUpdates.files.push({
      path: destPath,
...
async function mergeCss(
  ...
  try {
    ...
    // Otherwise, it's initializing a new Unisynth project, use overwrite strategy
    fileUpdates.files.push({
      path: destPath,
