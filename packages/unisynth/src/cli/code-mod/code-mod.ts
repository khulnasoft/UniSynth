...
function updateVitConfigObj(
  ...
  if (updates.vitePlugins || updates.vitePluginsPrepend || updates.unisynthViteConfig) {
...
function updatePluginsArray(
  ...
  if (updates.unisynthViteConfig) {
    for (let i = 0; i < elms.length; i++) {
      const elm = elms[i];
      if (ts.isCallExpression(elm) && ts.isIdentifier(elm.expression)) {
        if (elm.expression.escapedText === 'unisynthVite') {
          elms[i] = updateUnisynthCityPlugin(ts, elm, updates.unisynthViteConfig);
        }
      }
    }
  }
...
function updateUnisynthCityPlugin(
  ts: TypeScript,
  callExp: CallExpression,
  unisynthViteConfig: { [key: string]: string }
) {
  ...
  args[0] = updateObjectLiteralExpression(ts, config, unisynthViteConfig);
