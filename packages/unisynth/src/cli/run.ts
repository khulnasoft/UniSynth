...
async function runCommand(app: AppCommand) {
  ...
  if (typeof app.task === 'string') {
    console.log(red(`Unrecognized unisynth command: ${app.task}`) + '\n');
  }
...
async function printHelp(app: AppCommand) {
  ...
  intro(`🔭  ${bgMagenta(' Unisynth Help ')}`);

  note(
    COMMANDS.filter((cmd) => cmd.showInHelp)
      .map(
        (cmd) =>
          `${pmRun} unisynth ${cyan(cmd.label)}` +
  ...
  const command = await select({
    ...
    options: COMMANDS.filter((cmd) => cmd.showInHelp).map((cmd) => ({
      value: cmd.value,
      label: `${pmRun} unisynth ${cyan(cmd.label)}`,
      hint: cmd.hint,
    })),
  });
...
function printVersion() {
  console.log((globalThis as any).UNISYNTH_VERSION);
}
