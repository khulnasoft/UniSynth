...
import type { JSXNode } from '@khulnasoft.com/unisynth/jsx-runtime';
...
export const PrefetchServiceWorker = (opts: {
  ...
  const resolvedOpts = {
    ...
    scope: '/',
    verbose: false,
    path: 'unisynth-prefetch-service-worker.js',
    ...opts,
  };
  if (opts?.path?.startsWith?.('/')) {
    ...
    // baseUrl: '/'
    // path: 'unisynth-prefetch-service-worker.js'
    // the file 'unisynth-prefetch-service-worker.js' is not located in /build/
  ...
  const props = {
    dangerouslySetInnerHTML: [
      ...
      [
        ...
        'window.unisynthPrefetchSW||(window.unisynthPrefetchSW=[])',
...
export const PrefetchGraph = (
  ...
  const code = `(window.unisynthPrefetchSW||(window.unisynthPrefetchSW=[])).push(${args})`;
  const props = {
