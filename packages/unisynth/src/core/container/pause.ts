...
export const pauseContainer = async (
  ...
  eventsScript.textContent = `(window.unisynthevents||=[]).push(${extraListeners.join(', ')})`;
...
`
  ...
  const elementToIndex = new Map<Node | UnisynthElement, string | null>();
  const objs = Array.from(collector.$objSet$.keys());
  const objToId = new Map<any, string>();

  const getElementID = (el: UnisynthElement): string | null => {
  ...
  const mustGetObjId = (obj: any): string => {
    ...
    if (key === null) {
      ...
      // This as a workaround for https://github.com/khulnasoft/unisynth/issues/4979
      if (isQrl(obj)) {
...
const collectElement = (el: UnisynthElement, collector: Collector) => {
  const ctx = tryGetContext(el);
  if (ctx) {
    if (collector.$elements$.includes(ctx)) {
      return;
...
export const collectElementData = (
  elCtx: QContext,
  collector: Collector,
  dynamicCtx: UnisynthElement | boolean
) => {
    ...
    switch (objType) {
      ...
      case 'object': {
        ...
        if (target) {
          ...
          // Issue: https://github.com/khulnasoft/unisynth/issues/5001
...
const getQId = (el: UnisynthElement): string | null => {
  const ctx = tryGetContext(el);
  if (ctx) {
    return ctx.$id$;
  }
