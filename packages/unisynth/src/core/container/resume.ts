...
import { isComment, isElement, isNode, isUnisynthElement, isText } from '../util/element';
...
export const resumeIfNeeded = (containerEl: Element): void => {
  ...
  if (isResumed === 'paused') {
    ...
    if (qSerialize) {
      appendkhulnasoftTools(containerEl);
    }
  }
};
...
export const getPauseState = (containerEl: Element): SnapshotState | undefined => {
  ...
  const script = getUnisynthJSON(parentJSON, 'type');
  if (script) {
...
export const _deserializeData = (data: string, element?: unknown) => {
  ...
  if (isNode(element) && isUnisynthElement(element)) {
...
export const resumeContainer = (containerEl: Element) => {
  ...
  const pauseState =
    (containerEl as any)['_unisynthjson_'] ?? (getPauseState(containerEl) as SnapshotState);

  (containerEl as any)['_unisynthjson_'] = null;
  if (!pauseState) {
    logWarn('Skipping resuming unisynth/json metadata was not found.');
    return;
  }
...
export const getUnisynthJSON = (
  parentElm: Element,
  attribute: string
): HTMLScriptElement | undefined => {
  let child = parentElm.lastElementChild;
  while (child) {
    if (child.tagName === 'SCRIPT' && directGetAttribute(child, attribute) === 'unisynth/json') {
...
export const appendkhulnasoftTools = (containerEl: Element) => {
  (containerEl as any)['unisynth'] = {
