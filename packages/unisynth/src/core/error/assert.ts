import type { UnisynthElement, VirtualElement } from '../render/dom/virtual-element';
import { isElement, isUnisynthElement } from '../util/element';
import { throwErrorAndStop } from '../util/log';
import { qDev } from '../util/qdev';

const ASSERT_DISCLAIMER = 'Internal assert, this is likely caused by a bug in Unisynth: ';
...
export function assertUnisynthElement(el: any): asserts el is UnisynthElement {
  if (qDev) {
    if (!isUnisynthElement(el)) {
      console.error('Not a Unisynth Element, got', el);
      throwErrorAndStop(ASSERT_DISCLAIMER + 'Not a Unisynth Element');
    }
  }
}
