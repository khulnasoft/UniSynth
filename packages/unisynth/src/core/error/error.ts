...
export const codeToText = (code: number, ...parts: any[]): string => {
  if (qDev) {
    ...
    const MAP = [
      ...
      '<div> component can only be used at the root of a Unisynth component$()', // 18
      'Props are immutable by default.', // 19
      `Calling a 'use*()' method outside 'component$(() => { HERE })' is not allowed. 'use*()' methods provide hooks to the 'component$' state and lifecycle, ie 'use' hooks can only be called synchronously within the 'component$' function or another 'use' method.\nSee https://unisynth.dev/docs/components/tasks/#use-method-rules`, // 20
    ...
    return `Code(${code}) https://github.com/khulnasoft/unisynth/blob/main/packages/unisynth/src/core/error/error.ts#L${8 + code}`;
  }
};
