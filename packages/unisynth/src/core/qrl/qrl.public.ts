 ...
 *
 * `QRL` is an opaque token that is generated by the Unisynth Optimizer. (Do not rely on any
 * properties in `QRL` as it may change between versions.)
 *
 * ## Creating `QRL` references
 *
 * Creating `QRL` is done using `$(...)` function. `$(...)` is a special marker for the Unisynth
 ...
 * ```
 *
 * In the above code, the Unisynth Optimizer detects `$(...)` and transforms the code as shown
 * below:
 *
 * ```tsx
 * // FILE: <current file>
 ...
 * ```
 *
 * NOTE: `qrl(...)` is a result of Unisynth Optimizer transformation. You should never have to
 * invoke this function directly in your application. The `qrl(...)` function should be invoked only
 * after the Unisynth Optimizer transformation.
 *
 * ## Using `QRL`s
 *
 ...
 *
 * Use `$(...)` to tell Unisynth Optimizer to extract the expression in `$(...)` into a
 * lazy-loadable resource referenced by `QRL`.
 *
