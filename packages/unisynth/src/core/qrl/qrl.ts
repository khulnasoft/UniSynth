 ...
 *
 * This function should be used by the Unisynth Optimizer only. The function should not be directly
...
export const _regSymbol = (symbol: any, hash: string) => {
  if (typeof (globalThis as any).__unisynth_reg_symbols === 'undefined') {
    (globalThis as any).__unisynth_reg_symbols = new Map<string, any>();
  }
  (globalThis as any).__unisynth_reg_symbols.set(hash, symbol);
  return symbol;
};
