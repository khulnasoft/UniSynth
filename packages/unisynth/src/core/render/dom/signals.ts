...
import type { UnisynthElement } from './virtual-element';
...
import { isUnisynthElement } from '../../util/element';
      ...
      case 4: {
        ...
        if (!staticCtx.$visited$.includes(elm)) {
          ...
          if (isPromise(newVnode)) {
            ...
            const element = getUnisynthElement(operation[1]);
...
function getUnisynthElement(element: UnisynthElement | Text): UnisynthElement {
  while (element) {
    if (isUnisynthElement(element)) {
      return element;
    }
