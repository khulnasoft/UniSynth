...
export interface VirtualElement {
  ...
  readonly insertBeforeTo: (newParent: UnisynthElement, child: Node | null) => void;
  readonly appendTo: (newParent: UnisynthElement) => void;
  ...
  readonly querySelector: (query: string) => UnisynthElement | null;
  readonly querySelectorAll: (query: string) => UnisynthElement[];
...
export type UnisynthElement = Element | VirtualElement;
...
export class VirtualElementImpl implements VirtualElement {
  ...
  insertBeforeTo(newParent: UnisynthElement, child: Node | null) {
    // const ch = this.childNodes;
    const ch = this.childNodes;
    // TODO
  ...
  appendTo(newParent: UnisynthElement) {
  ...
  querySelectorAll(query: string) {
    const result: UnisynthElement[] = [];
    const ch = getChildren(this, isNodeElement);
    ch.forEach((el) => {
      if (isUnisynthElement(el)) {
        if (el.matches(query)) {
          result.push(el);
        }
