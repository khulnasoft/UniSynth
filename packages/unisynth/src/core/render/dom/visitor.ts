...
export const smartUpdateChildren = (
  ...
  assertUnisynthElement(oldVnode.$elm$);
...
// export const getChildren = (elm: UnisynthElement, mode: ChildrenMode): (Node | VirtualElement)[] => {
    ...
    if (props !== EMPTY_OBJ) {
      ...
      for (const prop in props) {
        ...
        if (isOnProp(prop)) {
          ...
          addUnisynthEvent(staticCtx, elm, normalized);
          continue;
        }
...
export const createElm = (
  ...
  if (!isVirtual) {
    ...
    for (const listener of elCtx.li) {
      addUnisynthEvent(staticCtx, elm, listener[0]);
    }
    const setsInnerHTML = props[dangerouslySetInnerHTML] !== undefined;
    if (setsInnerHTML) {
...
interface SlotMaps {
  slots: Record<string, UnisynthElement>;
...
export const smartSetProperty = (
  ...
  if (prop.startsWith(PREVENT_DEFAULT)) {
    registerUnisynthEvent(prop.slice(PREVENT_DEFAULT.length));
  }
...
export const directAppendChild = (parent: UnisynthElement, child: Node | VirtualElement) => {
...
export const directRemoveChild = (parent: UnisynthElement, child: Node | VirtualElement) => {
  if (isVirtualElement(child)) {
    child.remove();
  } else {
...
export const addUnisynthEvent = (staticCtx: RenderStaticContext, elm: UnisynthElement, prop: string) => {
  ...
  registerUnisynthEvent(prop);
};

export const registerUnisynthEvent = (prop: string) => {
  if (!qTest) {
    const eventName = getEventName(prop);
    try {
      // This is managed by unisynth-loader
      ((globalThis as any).unisynthevents ||= []).push(eventName);
    } catch (err) {
      logWarn(err);
    }
  }
