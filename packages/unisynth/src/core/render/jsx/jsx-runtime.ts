...
import type { UnisynthJSX } from './types/jsx-unisynth';
...
import { isUnisynthComponent, type OnRenderFn } from '../../component/component.public';
...
import type { JSXChildren } from './types/jsx-unisynth-attributes';
...
const validateJSXNode = (node: JSXNodeInternal) => {
  if (qDev) {
    ...
    invoke(undefined, () => {
      const isUnisynthC = isUnisynthComponent(type);
      ...
      if (!qRuntimeQrl) {
        for (const [prop, value] of allProps) {
          ...
          if (prop !== 'children' && isUnisynthC && value) {
            verifySerializable(
              value,
...
See https://unisynth.dev/docs/components/styles/#usestyles for more information.`);
          }
        }
        if (type === 'script') {
          if (children) {
...
export type { UnisynthJSX as JSX };
