...
import type { ClassList, JSXChildren } from '../jsx/types/jsx-unisynth-attributes';
      ...
      (stream) => {
        if (elCtx.$flags$ & HOST_FLAG_NEED_ATTACH_LISTENER) {
          ...
          for (const listener of groups) {
            ...
            registerUnisynthEvent(eventName, rCtx.$static$.$containerState$);
          }
...
This goes against the HTML spec: https://html.spec.whatwg.org/multipage/dom.html#interactive-content`,
    ...
    if (qDev && qInspector && node.dev && !(flags & IS_HEAD)) {
      ...
      if (sanitizedFileName && !/data-unisynth-inspector/.test(openingElement)) {
        openingElement += ` data-unisynth-inspector="${escapeHtml(
...
const processData = (
  ...
  if (isString(node) || typeof node === 'number') {
    ...
    const hostEl = rCtx.$cmpCtx$?.$element$ as UnisynthElement;
    let value;
    if (hostEl) {
      if (!insideText) {
...
export const registerUnisynthEvent = (prop: string, containerState: ContainerState) => {
