...
const _verifySerializable = <T>(value: T, seen: Set<any>, ctx: string, preMessage?: string): T => {
  ...
  if (shouldSerialize(unwrapped)) {
    ...
    if (typeObj === 'object') {
      message += ` because it's an instance of "${value?.constructor.name}". You might need to use 'noSerialize()' or use an object literal instead. Check out https://unisynth.dev/docs/advanced/dollar/`;
    } else if (typeObj === 'function') {
      const fnName = (value as Function).name;
      message += ` because it's a function named "${fnName}". You might need to convert it to a QRL using $(fn):\n\nconst ${fnName} = $(${String(
        value
      )});\n\nPlease check out https://unisynth.dev/docs/advanced/qrl/ for more information.`;
    }
...
export type SubscriberC = readonly [
  ...
  signal: Signal,
  elm: Node | string | UnisynthElement,
];
