...
import { assertUnisynthElement } from '../error/assert';
...
/** Unisynth Context of an element. */
export interface QContext {
  /** VDOM element. */
  $element$: UnisynthElement;
  $refMap$: any[];
  $flags$: number;
...
export const getContext = (el: UnisynthElement, containerState: ContainerState): QContext => {
  assertUnisynthElement(el);
