import { isUnisynthComponent } from '../component/component.public';
...
export const createSignal: UseSignal = <STATE>(initialState?: STATE): Signal<STATE> => {
  ...
  const value =
    isFunction(initialState) && !isUnisynthComponent(initialState)
...
export const useConstant = <T>(value: (() => T) | T): T => {
  ...
  value = isFunction(value) && !isUnisynthComponent(value) ? value() : value;
