...
export const isUnisynthElement = (value: object): value is UnisynthElement => {
  const nodeType = (value as any).nodeType;
  return nodeType === 1 || nodeType === 111;
};

export const isNodeElement = (value: object): value is UnisynthElement => {
  const nodeType = (value as any).nodeType;
...
export const isText = (value: Node | UnisynthElement): value is Text => {
  return (value as any).nodeType === 3;
};

export const isComment = (value: Node | UnisynthElement): value is Comment => {
