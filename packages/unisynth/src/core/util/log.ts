...
export const logWarn = (message?: any, ...optionalParams: any[]) => {
  if (qDev) {
    console.warn('%cUNISYNTH WARN', STYLE, message, ...printParams(optionalParams));
  }
};
...
export const logDebug = (message?: string, ...optionalParams: any[]) => {
  if (qDev) {
    ...
    console.debug('%cUNISYNTH', STYLE, message, ...printParams(optionalParams));
  }
};

export const tryGetContext = (element: UnisynthElement): QContext | undefined => {
...
const createAndLogError = (asyncThrow: boolean, message?: any, ...optionalParams: any[]) => {
  ...
  console.error('%cUNISYNTH ERROR', STYLE, err.message, ...printParams(optionalParams), err.stack);
  ...
  asyncThrow &&
    ...
    setTimeout(() => {
      ...
      // reporting it to a third-party tools such as Unisynth Insights, Sentry or New Relic.
      throw err;
    }, 0);
  return err;
};
