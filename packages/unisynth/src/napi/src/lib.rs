...
fn transform_fs(ctx: CallContext) -> Result<JsUnknown> {
	...
	let config: unisynth_core::TransformFsOptions = ctx.env.from_js_value(opts)?;

	let result = unisynth_core::transform_fs(config).unwrap();
...
fn transform_modules(ctx: CallContext) -> Result<JsUnknown> {
	let opts = ctx.get::<JsObject>(0)?;
	let config: unisynth_core::TransformModulesOptions = ctx.env.from_js_value(opts)?;

	let result = unisynth_core::transform_modules(config).unwrap();
	ctx.env.to_js_value(&result)
}
