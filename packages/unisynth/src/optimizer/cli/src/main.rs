...
use unisynth_core::{transform_fs, EmitMode, EntryStrategy, MinifyMode, TransformFsOptions};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	let matches = Command::new("unisynth")
        .version("1.0")
        ...
        .about("Unisynth CLI allows to optimize unisynth projects before bundling")
        .subcommand(
            Command::new("optimize")
                .about("takes a source directory of unisynth code and outputs an optimized version that lazy loads")
                .arg_required_else_help(true)
                .arg(
                    Arg::new("src")
                        .short('s')
...
) -> Result<unisynth_core::TransformOutput, Box<dyn std::error::Error>> {
