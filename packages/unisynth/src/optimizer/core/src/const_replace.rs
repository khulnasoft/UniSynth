	...
	pub fn new(is_server: bool, is_dev: bool, global_collector: &GlobalCollect) -> Self {
		Self {
			...
			is_server_ident: global_collector
				.get_imported_local(&IS_SERVER, &BUILDER_IO_UNISYNTH_BUILD),
			is_browser_ident: global_collector
				.get_imported_local(&IS_BROWSER, &BUILDER_IO_UNISYNTH_BUILD),
			is_dev_ident: global_collector.get_imported_local(&IS_DEV, &BUILDER_IO_UNISYNTH_BUILD),
		}
	}
}
macro_rules! id_eq {
