...
import { isBrowser, isServer, isDev } from '@khulnasoft.com/unisynth/build';
import * as unisynthCity from '@unisynth-city-plan';
...
const useUnisynthCityEnv = () => noSerialize(useServerData('unisynthcity'));
const UnisynthCityProvider = /* @__PURE__ */ componentQrl(
  /* @__PURE__ */ inlinedQrl((props) => {
    useStylesQrl(
      /* @__PURE__ */ inlinedQrl(
        `:root{view-transition-name: none}`,
        'UnisynthCityProvider_component_useStyles_RPDJAz33WLA'
      )
    );
    const env = useUnisynthCityEnv();
    if (!env?.params) throw new Error(`Missing Unisynth City Env Data`);
    const urlEnv = useServerData('url');
    if (!urlEnv) throw new Error(`Missing Unisynth URL Env Data`);
    ...
    const goto = eventQrl(
      /* @__PURE__ */ inlinedQrl(
        ...
        'UnisynthCityProvider_component_goto_event_cBcjROynRVg',
    ...
    useTaskQrl(
      /* @__PURE__ */ inlinedQrl(
          ...
          async function run() {
            ...
            if (isServer) {
              ...
              if (trackUrl.pathname.endsWith('/')) {
                if (!unisynthCity.trailingSlash) trackUrl.pathname = trackUrl.pathname.slice(0, -1);
              } else if (unisynthCity.trailingSlash) trackUrl.pathname += '/';
        ...
        'UnisynthCityProvider_component_useTask_02wMImzEAbk',
        [
          actionState,
          content,
          contentInternal,
  ...
  }, 'UnisynthCityProvider_component_TxCFOy819ag')
);
const UnisynthCityMockProvider = /* @__PURE__ */ componentQrl(
  /* @__PURE__ */ inlinedQrl((props) => {
    ...
    }, 'UnisynthCityMockProvider_component_goto_BUbtvTyvVRE');
    const documentHead = useStore(createDocumentHead, {
      deep: false,
    });
    const content = useStore(
  ...
  }, 'UnisynthCityMockProvider_component_WmYC5H00wtI')
);
...
const globalActionQrl = (actionQrl, ...rest) => {
  ...
  if (isServer) {
    if (typeof globalThis._unisynthActionsMap === 'undefined')
      globalThis._unisynthActionsMap = /* @__PURE__ */ new Map();
    globalThis._unisynthActionsMap.set(action.__id, action);
  }
  return action;
};
...
const routeLoaderQrl = (loaderQrl, ...rest) => {
  ...
  function loader() {
    ...
    For more information check: https://unisynth.dev/unisynthcity/route-loader/`);
  ...
  function stuff() {
    return /* @__PURE__ */ inlinedQrl(
      async (...args) => {
        ...
        if (isServer) {
          const requestEvent = useUnisynthCityEnv()?.ev;
          ...
          const res = await fetch(path, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/unisynth-json',
              'X-QRL': hash,
            },
            body,
          });
          ...
          } else if (contentType === 'application/unisynth-json') {
