...
use crate::transform::{Segment, SegmentKind, UnisynthTransform, UnisynthTransformOptions};
...
#[derive(Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
pub struct UnisynthManifest {
	pub version: JsWord,
	pub symbols: HashMap<JsWord, SegmentAnalysis>,
	pub bundles: HashMap<JsWord, UnisynthBundle>,
...
impl TransformOutput {
	...
	pub fn get_manifest(&self) -> UnisynthManifest {
		let mut manifest = UnisynthManifest {
			bundles: HashMap::new(),
			symbols: HashMap::new(),
			mapping: HashMap::new(),
			version: "1".into(),
		...
		Ok((program, comments, is_type_script, is_jsx)) => {
			...
			swc_common::GLOBALS.set(&Globals::new(), || {
				swc_common::errors::HANDLER.set(&handler, || {
					...
					if transpile_jsx && is_jsx {
						...
						if is_jsx {
							...
							react_options.import_source =
								Some("@khulnasoft.com/unisynth".to_string());
						};
					...
					let mut qt: Option<UnisynthTransform<'_>> = None;
					...
					if config.mode != EmitMode::Lib {
						...
						// split into segments
						let mut unisynth_transform =
							UnisynthTransform::new(UnisynthTransformOptions {
								path_data: &path_data,
						...
						program = program.fold_with(&mut unisynth_transform);
						...
						if matches!(
							...
							program.visit_mut_with(&mut SideEffectVisitor::new(
								&unisynth_transform.options.global_collect,
								&path_data,
								config.src_dir,
							));
						...
						segments = unisynth_transform.segments.clone();
						qt = Some(unisynth_transform);
					}
