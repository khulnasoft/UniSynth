...
pub struct UnisynthTransform<'a> {
	pub segments: Vec<Segment>,
	pub options: UnisynthTransformOptions<'a>,
...
pub struct UnisynthTransformOptions<'a> {
...
impl<'a> UnisynthTransform<'a> {
	pub fn new(options: UnisynthTransformOptions<'a>) -> Self {
		...
		let jsx_functions = options
			...
			.flat_map(|(id, import)| {
				match (
					...
					(ImportKind::Named, "@khulnasoft.com/unisynth", "jsx") => Some(id.clone()),
					(ImportKind::Named, "@khulnasoft.com/unisynth", "jsxs") => Some(id.clone()),
					(ImportKind::Named, "@khulnasoft.com/unisynth", "jsxDEV") => Some(id.clone()),
					(ImportKind::Named, "@khulnasoft.com/unisynth/jsx-runtime", _) => {
						Some(id.clone())
					}
					(ImportKind::Named, "@khulnasoft.com/unisynth/jsx-dev-runtime", _) => {
						Some(id.clone())
					}
					_ => None,
				}
			...
			.flat_map(|(id, import)| {
				match (
					...
					(
						...
						| "@khulnasoft.com/unisynth/jsx-dev-runtime",
						"Fragment",
					) => Some(id.clone()),
					(
						ImportKind::Named,
					...
					(ImportKind::Named, "@khulnasoft.com/unisynth-city", "Link") => {
						Some(id.clone())
					}
					(_, source, _) => {
...
impl<'a> Fold for UnisynthTransform<'a> {
	noop_fold_type!();
