...
function prioritizeSymbolNames(manifest: UnisynthManifest) {
...
function sortBundleNames(manifest: UnisynthManifest) {
...
function updateSortAndPriorities(manifest: UnisynthManifest) {
  const prioritizedSymbolNames = prioritizeSymbolNames(manifest);
  const prioritizedSymbols: UnisynthManifest['symbols'] = {};
  const prioritizedMapping: UnisynthManifest['mapping'] = {};
  ...
  const sortedBundles: { [fileName: string]: UnisynthBundle } = {};
...
export function getValidManifest(manifest: UnisynthManifest | undefined | null) {
  if (
    manifest != null &&
