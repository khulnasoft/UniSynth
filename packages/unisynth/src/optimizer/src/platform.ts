...
import { UNISYNTH_BINDING_MAP } from './unisynth-binding-map';
...
export async function getSystem() {
  ...
  const sys: OptimizerSystem = {
    dynamicImport: (path) => {
      throw new Error(
        `Unisynth Optimizer sys.dynamicImport() not implemented, trying to import: "${path}"`
      );
    },
    strictDynamicImport: (path) => {
      throw new Error(
        `Unisynth Optimizer sys.strictDynamicImport() not implemented, trying to import: "${path}"`
      );
    },
    path: null as any,
    cwd: () => '/',
...
export async function loadPlatformBinding(sys: OptimizerSystem) {
  ...
  if (sysEnv === 'node' || sysEnv === 'bun') {
    // Node.js
    const platform = (UNISYNTH_BINDING_MAP as any)[process.platform];
    if (platform) {
  ...
  if (globalThis.IS_CJS) {
    ...
    if (sysEnv === 'node' || sysEnv === 'bun') {
      // CJS WASM Node.js
      const wasmPath = sys.path.join(__dirname, '..', 'bindings', 'unisynth_wasm_bg.wasm');
      const mod = await sys.dynamicImport(`../bindings/unisynth.wasm.cjs`);
      const fs: typeof import('fs') = await sys.dynamicImport('node:fs');
    ...
    if (sysEnv === 'webworker' || sysEnv === 'browsermain') {
      // CJS WASM Browser
      let version = versions.unisynth;
      const cachedCjsCode = `unisynthWasmCjs${version}`;
      const cachedWasmRsp = `unisynthWasmRsp${version}`;

      let cjsCode: string = (globalThis as any)[cachedCjsCode];
      let wasmRsp: Response = (globalThis as any)[cachedWasmRsp];

      if (!cjsCode || !wasmRsp) {
        version = versions.unisynth.split('-dev')[0];
        const cdnUrl = `https://cdn.jsdelivr.net/npm/@khulnasoft.com/unisynth@${version}/bindings/`;
        const cjsModuleUrl = new URL(`./unisynth.wasm.cjs`, cdnUrl).href;
        const wasmUrl = new URL(`./unisynth_wasm_bg.wasm`, cdnUrl).href;

        const rsps = await Promise.all([fetch(cjsModuleUrl), fetch(wasmUrl)]);

        for (const rsp of rsps) {
          if (!rsp.ok) {
            throw new Error(`Unable to fetch Unisynth WASM binding from ${rsp.url}`);
          }
        }
  ...
  if (globalThis.IS_ESM) {
    if (sysEnv === 'node' || sysEnv === 'bun') {
      ...
      const wasmPath = sys.path.join(__dirname, '..', 'bindings', 'unisynth_wasm_bg.wasm');
      const mod = await sys.dynamicImport(`../bindings/unisynth.wasm.mjs`);
      const fs: typeof import('fs') = await sys.dynamicImport('node:fs');
      ...
      const module = await sys.dynamicImport(`../bindings/unisynth.wasm.mjs`);
