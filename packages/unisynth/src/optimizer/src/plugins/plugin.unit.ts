...
import { unisynthVite } from './vite';

const cwd = process.cwd();

test('types', () => () => {
  unisynthVite({ csr: true });
  unisynthVite({ csr: false, ssr: {} });
});
...
test('defaults (buildMode: production)', async () => {
  ...
  assert.deepEqual(opts.resolveUnisynthBuild, true);
  assert.deepEqual(opts.debug, false);
  assert.deepEqual(opts.rootDir, normalizePath(cwd));
  assert.deepEqual(opts.tsconfigFileNames, ['./tsconfig.json']);
  assert.deepEqual(opts.input, [normalizePath(resolve(cwd, 'src', 'root'))]);
...
test('manifestInput', async () => {
  const plugin = await mockPlugin();
  const manifestInput: UnisynthManifest = {
    manifestHash: '',
    mapping: {},
    symbols: {},
    bundles: {},
...
test('resolveUnisynthBuild true', async () => {
  const plugin = await mockPlugin();
  const opts = plugin.normalizeOptions({ resolveUnisynthBuild: true });
  assert.deepEqual(opts.resolveUnisynthBuild, true);
});

test('resolveUnisynthBuild false', async () => {
  const plugin = await mockPlugin();
  const opts = plugin.normalizeOptions({ resolveUnisynthBuild: false });
  assert.deepEqual(opts.resolveUnisynthBuild, false);
});
