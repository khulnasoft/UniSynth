...
import { unisynthRollup } from './rollup';
import type { Rollup } from 'vite';
import type { OptimizerOptions } from '../types';
import type { NormalizedUnisynthPluginOptions } from './plugin';
...
test('rollup default input options, client', async () => {
  ...
  const plugin = unisynthRollup(initOpts);
...
test('rollup default input options, ssr', async () => {
  ...
  const plugin = unisynthRollup(initOpts);
  const rollupInputOpts: Rollup.InputOptions = await plugin.options!({});
  const opts: NormalizedUnisynthPluginOptions = plugin.api.getOptions();
