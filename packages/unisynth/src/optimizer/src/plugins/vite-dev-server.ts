...
import { parseId, type NormalizedUnisynthPluginOptions } from './plugin';
import type { UnisynthViteDevResponse } from './vite';
...
function createSymbolMapper(base: string): SymbolMapperFn {
  return (
    ...
    if (!parent) {
      console.error(
        'unisynth vite-dev-server symbolMapper: unknown qrl requested without parent:',
        symbolName
      );
...
export async function configureDevServer(
  ...
  const hasUnisynthCity = server.config.plugins?.some(
    (plugin) => plugin.name === 'vite-plugin-unisynth-city'
  );
  // unisynth middleware injected BEFORE vite internal middlewares
  server.middlewares.use(async (req, res, next) => {
    try {
      ...
      if (shouldSsrRender(req, url)) {
        const { _unisynthEnvData } = res as UnisynthViteDevResponse;
        if (!_unisynthEnvData && hasUnisynthCity) {
          console.error(`not SSR rendering ${url} because Unisynth City Env data did not populate`);
          res.statusCode ||= 404;
          res.setHeader('Content-Type', 'text/plain');
          res.writeHead(res.statusCode);
          res.end('Not a SSR URL according to Unisynth City');
          return;
        }
        ...
        if (isClientDevOnly) {
          ...
          res.setHeader('X-Powered-By', 'Unisynth Vite Dev Server');
      ...
      if (typeof (res as UnisynthViteDevResponse)._unisynthRenderResolve === 'function') {
        (res as UnisynthViteDevResponse)._unisynthRenderResolve!();
      }
    }
  });
...
const shouldSsrRender = (req: IncomingMessage, url: URL) => {
  ...
  if (pathname.includes('@khulnasoft.com/unisynth/build')) {
    return false;
  }
...
const DEV_UNISYNTH_INSPECTOR = (opts: NormalizedUnisynthPluginOptions['devTools'], srcDir: string) => {
  ...
  return (
    `<script>
      globalThis.unisynthdevtools = ${JSON.stringify(unisynthdevtools)};
    </script>` +
...
const END_SSR_SCRIPT = (opts: NormalizedUnisynthPluginOptions, srcDir: string) => `
<style>${VITE_ERROR_OVERLAY_STYLES}</style>
<script type="module" src="/@vite/client"></script>
${errorHost}
${perfWarning}
${DEV_UNISYNTH_INSPECTOR(opts.devTools, srcDir)}
`;
...
export const VITE_DEV_CLIENT_QS = `unisynth-vite-dev-client`;
