...
test('baseURI', () => {
  const glb = ensureGlobals({ nodeType: 9 }, { url: 'http://my.unisynth.dev/my-path' });
  assert.equal(glb.document.baseURI, 'http://my.unisynth.dev/my-path');

  glb.document.baseURI = 'http://my.unisynth.dev/new-path';
  assert.equal(glb.document.baseURI, 'http://my.unisynth.dev/new-path');
});
...
test('origin, no options', () => {
  const glb = ensureGlobals({ nodeType: 9 }, {});
  assert.equal(glb.origin, 'http://document.unisynth.dev');
});
