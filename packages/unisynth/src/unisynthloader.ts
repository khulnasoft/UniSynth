...
export const unisynthLoader = (
  ...
  const resolveContainer = (containerEl: QContainerElement) => {
    if (containerEl[Q_JSON] === undefined) {
      ...
      while (script) {
        if (script.tagName === 'SCRIPT' && script[getAttribute]('type') === 'unisynth/json') {
      ...
      for (const qrl of attrValue.split('\n')) {
        ...
        if (element[isConnected]) {
          try {
            ...
            isSync || emitEvent<UnisynthSymbolEvent>('qsymbol', { ...eventData });
        ...
        const observer = new IntersectionObserver((entries) => {
          for (const entry of entries) {
            if (entry.isIntersecting) {
              ...
              dispatch(entry[target], '', createEvent<UnisynthVisibleEvent>(qvisible, entry));
            }
          }
        });
  ...
  if (!(Q_CONTEXT in doc)) {
    // Mark unisynth-loader presence but falsy
    doc[Q_CONTEXT] = 0;
    const unisynthevents = win.unisynthevents;
    // If `unisynthEvents` is an array, process it.
    if (Array.isArray(unisynthevents)) {
      processEventOrNode(...unisynthevents);
    }
    // Now rig up `unisynthEvents` so we get notified of new registrations by other containers.
    win.unisynthevents = {
      events: events,
      roots: roots,
...
export interface UnisynthLoaderMessage extends MessageEvent {
  data: string[];
}
