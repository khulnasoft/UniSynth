...
export async function generateUnisynthCityApiMarkdownDocs(
  config: BuildConfig,
  apiJsonInputDir: string
) {
  await generateApiMarkdownPackageDocs(config, apiJsonInputDir, ['unisynth-city']);
  await generateApiMarkdownPackageDocs(config, apiJsonInputDir, ['unisynth-city', 'middleware']);
  await generateApiMarkdownPackageDocs(config, apiJsonInputDir, ['unisynth-city', 'static']);
  await generateApiMarkdownPackageDocs(config, apiJsonInputDir, ['unisynth-city', 'vite']);

  // doesn't really belong here, ah well
  await generateApiMarkdownPackageDocs(config, apiJsonInputDir, ['unisynth-react']);
}
...
async function createApiData(
  ...
  apiData.members.forEach((m) => {
    m.content = m.content.replace(/\.\/unisynth(.*)\.md/g, '#');
  });
...
function getMdFile(subPkgName: string, hierarchy: string[]) {
  ...
  return `unisynth${subPkgName.includes('city') ? '-city' : ''}${mdFile}.md`;
}
...
function getEditUrl(config: BuildConfig, fileUrlPath: string | undefined) {
  if (fileUrlPath) {
    ...
    if (existsSync(tsxPath)) {
      const url = new URL(rootRelPath, `https://github.com/khulnasoft/unisynth/tree/main/`);
