  ...
  await new Promise((resolve, reject) => {
    try {
      ...
      const args = [
        ...
        'unisynth_napi',
        `--platform`,
        `--config=packages/unisynth/src/napi/napi.config.json`,
...
export async function copyPlatformBindingWasm(config: BuildConfig) {
  ...
  try {
    const releaseDataUrl = `https://data.jsdelivr.com/v1/package/npm/@khulnasoft.com/unisynth`;
  ...
  try {
    ...
    const bindingFilenames = [
      'unisynth.darwin-arm64.node',
      'unisynth.darwin-x64.node',
      'unisynth.wasm.cjs',
      'unisynth.wasm.mjs',
      'unisynth.win32-x64-msvc.node',
      'unisynth_wasm_bg.wasm',
    ];
    ...
    await Promise.all(
      bindingFilenames.map(async (bindingFilename) => {
        ...
        if (!existsSync(cachedPath)) {
          const cdnUrl = `https://cdn.jsdelivr.net/npm/@khulnasoft.com/unisynth@${buildVersion}/bindings/${bindingFilename}`;
