...
export async function buildWasmBinding(config: BuildConfig) {
  const srcWasmDir = join(config.srcUnisynthDir, `wasm`);
  ...
  async function buildForTarget(env = {}) {
    ...
    return join(tmpBuildDir, 'unisynth_wasm.js');
  }
  ...
  await build.write({
    format: 'es',
    file: join(config.distBindingsDir, 'unisynth.wasm.mjs'),
    exports: 'named',
  });

  await build.write({
    format: 'cjs',
    file: join(config.distBindingsDir, 'unisynth.wasm.cjs'),
    exports: 'named',
  });

  await copyFile(
    join(tmpBuildDir, 'unisynth_wasm_bg.wasm'),
    join(config.distBindingsDir, 'unisynth_wasm_bg.wasm')
  );
