...
const PACKAGE = 'create-unisynth';

export async function buildCreateUnisynthCli(config: BuildConfig) {
  const srcCliDir = join(config.packagesDir, PACKAGE);
  const distCliDir = join(srcCliDir, 'dist');

  await bundleCreateUnisynthCli(config, srcCliDir, distCliDir);
  await copyStartersDir(config, distCliDir, ['apps']);
  await syncBaseStarterVersionsFromUnisynth(config);

  console.log('🐠 create-unisynth cli');
}

async function bundleCreateUnisynthCli(config: BuildConfig, srcCliDir: string, distCliDir: string) {
  emptyDir(distCliDir);
...
export async function publishCreateUnisynthCli(
  config: BuildConfig,
  distTag: string,
  version: string,
  isDryRun: boolean
...
async function syncBaseStarterVersionsFromUnisynth(config: BuildConfig) {
  const unisynthVersion = await getUnisynthVersion(config);

  await updateBaseVersions(config, unisynthVersion);
}
...
async function updateBaseVersions(config: BuildConfig, version: string) {
  ...
  const semverUnisynth = config.devRelease ? `${version}` : `^${version}`;
  console.log(`   update devDependencies["@khulnasoft.com/unisynth"] = "${semverUnisynth}"`);
  baseAppPkg.devDependencies['@khulnasoft.com/unisynth'] = semverUnisynth;

  console.log(`   update devDependencies["@khulnasoft.com/unisynth-city"] = "${semverUnisynth}"`);
  baseAppPkg.devDependencies['@khulnasoft.com/unisynth-city'] = semverUnisynth;

  console.log(`   update devDependencies["eslint-plugin-unisynth"] = "${semverUnisynth}"`);
  baseAppPkg.devDependencies['eslint-plugin-unisynth'] = semverUnisynth;
...
async function updatePackageJson(config: BuildConfig, destDir: string) {
  ...
  if (pkgJson.devDependencies && pkgJson.devDependencies['@khulnasoft.com/unisynth']) {
    pkgJson.devDependencies['@khulnasoft.com/unisynth'] = unisynthVersion;
  }

  if (pkgJson.devDependencies && pkgJson.devDependencies['eslint-plugin-unisynth']) {
    pkgJson.devDependencies['eslint-plugin-unisynth'] = unisynthVersion;
  }
