...
import { rules, configs } from '../packages/eslint-plugin-unisynth/index';
import { examples, type UnisynthEslintExample } from '../packages/eslint-plugin-unisynth/examples';

const mdx = [];

const outputPathMdx = resolve(
  process.cwd(),
  'packages/docs/src/routes/docs/(unisynth)/advanced/eslint/index.mdx'
);
...
function renderExample(example: UnisynthEslintExample, state: 'good' | 'bad' = 'good') {
...
const rulesMap = Object.keys(rules).map((ruleName) => {
  ...
  return {
    ...
    recommended: configs?.recommended?.rules[`unisynth/${rule}`] || false,
    strict: configs.strict.rules[`unisynth/${rule}`] || false,
...
[//]: <> ( packages/eslint-plugin-unisynth/index.ts )
[//]: <> (--------------------------------------)
`);
...
mdx.push(
  '<p>Unisynth comes with its own set of ESLint rules to help developers write better code.</p>'
);
...
rulesMap.forEach((rule) => {
  ...
  Object.keys(rule.messages).map((messageKey) => {
    ...
    const goodExamples: UnisynthEslintExample[] = rule?.examples?.[messageKey]?.good || [];
    const badExamples: UnisynthEslintExample[] = rule?.examples?.[messageKey]?.bad || [];
    ...
    mdx.push(
      `<div class="edit-examples-wrapper"><a href="https://github.com/khulnasoft/unisynth/edit/main/packages/eslint-plugin-unisynth/src/${kebabToCamel(
        rule.name
