...
export async function submoduleBuild(config: BuildConfig) {
  ...
  const buildSrcDtsDir = join(config.dtsDir, 'packages', 'unisynth', 'src', submodule);
  const buildDestDir = join(config.distUnisynthPkgDir, submodule);
  ...
  const loaderPkg: PackageJSON = {
    name: `@khulnasoft.com/unisynth/build`,
...
export async function bundleIndex(config: BuildConfig, entryName: string) {
  ...
  const opts: BuildOptions = {
    entryPoints: [join(config.srcUnisynthDir, 'build', `${entryName}.ts`)],
  ...
  const cjs = build({
    ...
    banner: {
      js: `globalThis.unisynthBuild = (function (module) {`,
    },
    footer: {
