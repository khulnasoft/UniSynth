...
export function loadConfig(args: string[] = []) {
  ...
  const srcUnisynthDir = join(packagesDir, 'unisynth', 'src');
  const distUnisynthPkgDir = join(packagesDir, 'unisynth', 'dist');
  ...
  // rename unisynth to build
  (alias as any).build = 'unisynth';
  ...
  const config = mri<BuildConfig>(args, {
    ...
    default: {
      ...
      srcUnisynthDir,
      tmpDir,
      srcUnisynthCityDir: join(packagesDir, 'unisynth-city', 'src'),
      srcUnisynthLabsDir: join(packagesDir, 'unisynth-labs'),
      srcNapiDir: join(srcUnisynthDir, 'napi'),
      scriptsDir: join(rootDir, 'scripts'),
      startersDir: join(rootDir, 'starters'),
      distUnisynthPkgDir,
      distUnisynthCityPkgDir: join(packagesDir, 'unisynth-city', 'lib'),
      distBindingsDir: join(packagesDir, 'unisynth', 'bindings'),
 ...
 * found in the LICENSE file at https://github.com/khulnasoft/unisynth/blob/main/LICENSE
 */
`.trim();
};
...
export async function getUnisynthVersion(config: BuildConfig) {
  const unisynthDir = join(config.packagesDir, 'unisynth');
  const unisynthPkgJson = await readPackageJson(unisynthDir);
  return unisynthPkgJson.version;
}
