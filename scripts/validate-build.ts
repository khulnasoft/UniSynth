...
export async function validateBuild(config: BuildConfig) {
  ...
  const pkgPath = join(config.distUnisynthPkgDir, 'package.json');
  ...
  const expectedFiles = pkgFiles.map((f) => join(config.distUnisynthPkgDir, f));
  ...
  await Promise.all([
    validateModuleTreeshake(config, join(config.distUnisynthPkgDir, 'core.min.mjs')),
    validateModuleTreeshake(config, join(config.distUnisynthPkgDir, 'core.prod.mjs')),
    validateModuleTreeshake(config, join(config.distUnisynthPkgDir, 'core.mjs')),
    validateModuleTreeshake(config, join(config.distUnisynthPkgDir, 'server.mjs')),
  ]);
  ...
  getFiles(config.distUnisynthPkgDir);
...
async function validatePackageJson(config: BuildConfig, pkg: PackageJSON, errors: string[]) {
  async function validatePath(path: string) {
    try {
      await access(join(config.distUnisynthPkgDir, path));
    } catch (e: any) {
      errors.push(
...
async function validateModuleTreeshake(
  ...
  const bundle = await rollup({
    ...
    external: ['@khulnasoft.com/unisynth/build', '@khulnasoft.com/unisynth', ...external],
    plugins: [
      {
        name: 'resolver',
        resolveId(id) {
