...
test.describe("Unisynth City API", () => {
  test("Unisynth City API", async ({ page: api }) => {
    const rsp = (await api.goto("/unisynthcity-test/api/data.json"))!;
  ...
  test("Unisynth City API, params", async ({ page: api }) => {
    const rsp = (await api.goto("/unisynthcity-test/api/builder.io/oss.json"))!;
    expect(rsp.status()).toBe(200);
    expect(rsp.headers()["content-type"]).toBe(
      "application/json; charset=utf-8",
    );
  ...
  test("Page route GET, custom json response", async ({ page: api }) => {
    const rsp = (await api.goto("/unisynthcity-test/products/hat/?json=true"))!;
    expect(rsp.status()).toBe(200);
    expect(rsp.headers()["content-type"]).toBe(
      "application/json; charset=utf-8",
    );
  ...
  test("PUT endpoint on page", async ({ page }) => {
    const rsp = (await page.goto("/unisynthcity-test/api/"))!;
    expect(rsp.status()).toBe(200);
  ...
  test('redirect from product page because of "querystring-test" exists', async ({
    ...
    const rsp = (await page.goto(
      "/unisynthcity-test/products/hat/?querystring-test=true",
    ))!;
    expect(new URL(rsp.url()).pathname).toBe("/unisynthcity-test/");
  });
  ...
  test("page with a dot in the pathname, with a trailing slash", async ({
    ...
    const rsp = (await page.goto("/unisynthcity-test/issue2441/abc.page/"))!;
    expect(rsp.status()).toBe(200);
