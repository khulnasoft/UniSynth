...
test.describe("loaders", () => {
  ...
  function tests() {
    test("should run loaders", async ({ page }) => {
      await page.goto("/unisynthcity-test/loaders/hola");
      ...
      await submit.click();
      await expect(title).toHaveText("Loaders - ACTION: Manuel - Unisynth", {
        useInnerText: true,
      });
      await expect(date).toHaveText("date: 2021-01-01T00:00:00.000Z");
      await expect(slow).toHaveText("slow: 123");
    ...
    test("should pass reactivity issue", async ({ page }) => {
      await page.goto("/unisynthcity-test/issue-loader");
    ...
    test("serialization of loaders", async ({ page, javaScriptEnabled }) => {
      await page.goto("/unisynthcity-test/issue-loader-serialization/");
