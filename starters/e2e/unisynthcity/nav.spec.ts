...
test.describe("actions", () => {
  ...
  function spaOnlyTests() {
    test("issue4100", async ({ page }) => {
      await page.goto("/unisynthcity-test/issue4100/");
    ...
    test.describe("scroll-restoration", () => {
      test("should not refresh again on popstate after manual refresh", async ({
        ...
        await page.goto("/unisynthcity-test/scroll-restoration/page-long/");
      ...
      test("should scroll on hash change", async ({ page }) => {
        await page.goto("/unisynthcity-test/scroll-restoration/hash/");
        expect(page).toHaveURL("/unisynthcity-test/scroll-restoration/hash/");
        ...
        expect(page).toHaveURL(
          "/unisynthcity-test/scroll-restoration/hash/#hash-2",
        );
        let scrollY1;
        do {
        ...
        expect(page).toHaveURL(
          "/unisynthcity-test/scroll-restoration/hash/#hash-1",
        );
      ...
      test("should restore scroll on back and forward navigations", async ({
        ...
        // UnisynthCity relies on a debounced scroll handler to save scroll position.
      ...
      test("issue4502 (link)", async ({ page }) => {
        await page.goto("/unisynthcity-test/issue4502/");
        const count = page.locator("#count");
        await expect(count).toHaveText("Count: 0");
        await count.click();
        await expect(count).toHaveText("Count: 1");
        await page.locator("#link").click();
        await page.waitForURL("/unisynthcity-test/issue4502/broken/route/");
    ...
    test("issue 6660 internal params should not trigger navigation", async ({
      ...
      await page.goto("/unisynthcity-test/issue6660/");
      ...
      await page.goto("/unisynthcity-test/issue6660/?var=1&hello");
      await expect(page.locator("#status")).toBeHidden();
    ...
    test("preventNavigate", async ({ page }) => {
      await page.goto("/unisynthcity-test/prevent-navigate/");
      ...
      expect(new URL(page.url()).pathname).toBe("/unisynthcity-test/");
      await page.goBack();
      await expect(count).toHaveText("0");
      ...
      expect(new URL(page.url()).pathname).toBe(
        "/unisynthcity-test/prevent-navigate/",
      );
      await expect(confirmText).toContainText("/unisynthcity-test/?");
      await itemLink.click();
      await expect(confirmText).toContainText(
        "/unisynthcity-test/prevent-navigate/5/?",
      );
  ...
  function tests() {
    test.describe("issue2829", () => {
      test("should navigate with context", async ({ page }) => {
        await page.goto("/unisynthcity-test/issue2829/a/");
    ...
    test.describe("issue2890", () => {
      ...
      test("should navigate (link 1)", async ({ page, javaScriptEnabled }) => {
        ...
        await expect(toPath(page.url())).toEqual(
          "/unisynthcity-test/issue2890/b/?query=123",
        );
        await expect(page.locator("#loader")).toHaveText(
      ...
      test("should navigate (link 2)", async ({ page, javaScriptEnabled }) => {
        ...
        await expect(page.locator("h1")).toHaveText("Query");
        await expect(toPath(page.url())).toEqual(
          "/unisynthcity-test/issue2890/b/?query=321",
        );
        await expect(page.locator("#loader")).toHaveText(
      ...
      test("should navigate (link 3)", async ({ page, javaScriptEnabled }) => {
        ...
        await expect(page.locator("h1")).toHaveText("Query");
        await expect(toPath(page.url())).toEqual(
          "/unisynthcity-test/issue2890/b/?query=321&hash=true#h2",
        );
        await expect(page.locator("#loader")).toHaveText(
    ...
    test.describe("issue 2751", () => {
      test("should navigate without crash", async ({
        ...
        await assertPage(ctx, {
          pathname: "/unisynthcity-test/actions/",
          title: "Actions - Unisynth",
    ...
    test("issue4956", async ({ page }) => {
      await page.goto("/unisynthcity-test/issue4956?id=1");
    ...
    test("issue4792", async ({ page }) => {
      const site = "/unisynthcity-test/issue4792/";
    ...
    test("media in home page", async ({ page }) => {
      await page.goto("/unisynthcity-test/");
    ...
    test("redirects, re-runs loaders and changes the url within the same page when search params changed", async ({
      ...
      await page.goto("/unisynthcity-test/search-params-redirect/");
      await page.getByText("Submit").click();
      await page.waitForURL(
        "**/unisynthcity-test/search-params-redirect/?redirected=true",
      );
