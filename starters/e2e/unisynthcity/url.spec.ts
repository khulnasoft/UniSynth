...
test.describe("Unisynth City URL params", () => {
  test.describe("mpa", () => {
...
function tests() {
  test("Route Params", async ({ context }) => {
    ...
    const response = (await page.goto("/unisynthcity-test/usa/astoria/"))!;
    const status = response.status();
    expect(status).toBe(200);
    const title = page.locator("title");
    expect(await title.innerText()).toBe(
      "Weather: usa astoria, 30C, 10day - Unisynth",
    );
  ...
  test("Query Params", async ({ context }) => {
    ...
    const response = (await page.goto(
      "/unisynthcity-test/usa/hill-valley/?unit=F&forecast=24hour",
    ))!;
    const status = response.status();
    expect(status).toBe(200);
    const title = page.locator("title");
    expect(await title.innerText()).toBe(
      "Weather: usa hill-valley, 30F, 24hour - Unisynth",
    );
